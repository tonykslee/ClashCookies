...
- Includes `/accounts` and `/link list` rep-badge rendering from persisted `TrackedClanRep` + `TrackedClan.clanBadge` data, placing the clan badge before the TH icon in `/accounts` and replacing the linked checkmark in `/link list`. `/link list` also supports `visibility:public` for a shared, channel-visible roster where any server member can use the controls.
