...
- `/bot-logs type:clan-goals` configures live and canonical war-end clan-goal notification routing with clan-log, clan-lead, generic bot-log, custom, or disabled destinations; it defaults to disabled and only uses `channel` for custom routing. Active-war and war-end goal notifications remain non-pinging.
