...
- `/fwa layout` is the canonical FWA layout management/view surface. It associates FWA `(Townhall, Type)` designations with shared `LayoutRecord` lifecycle/post rows, keeps legacy `FwaLayouts.LayoutLink`/`ImageUrl` compatibility copies synchronized, and returns canonical Discord post jump links without echoing raw Clash URLs.
