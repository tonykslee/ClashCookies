...
- Includes `/sync readiness` for posting the shared FWA readiness dashboard, `/sync retrospective` for a private-by-default persisted alliance summary with explicit coverage, plus `/sync time post` for immediate sync announcements that also schedule the companion readiness dashboard 2 hours before sync.
