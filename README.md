...
- CWL now has parallel persisted owners for live/prep rounds, ended round history, and planner state so `/todo cwl`, `/cwl activity`, and `/cwl ...` commands stay DB-first when persisted state exists. `/cwl activity` reads persisted activity and CWL history without external API calls; manual baseline commands are retired and their legacy tables remain dormant pending staged cleanup.
