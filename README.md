...
- `/fwa base-swap clan:<tag> [war-bases:<positions>] [base-errors:<groups>] [fwa-bases:<positions>] [swap-reminder:<true|false>] [ping_role:<role>]` posts the tracked base-swap acknowledgement flow. `base-errors` can group positions with an optional note after the leading positions. Commas separate groups and cannot appear inside explanations; use semicolons or dashes instead if you need punctuation inside a note. The same note can apply to multiple positions in one group. Normal FWA affected-player DMs are automatic for linked, unacknowledged entries; `swap-reminder` only controls the optional CWL swap-back reminder section, so omitting it or setting it false does not disable ordinary FWA affected-player DMs. The post keeps the usual TH layout links, reminder section, and DM copy lines.
