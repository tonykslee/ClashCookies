...
- Includes `npm run backfill:membership-history-participation`, an explicit-sync, dry-run-by-default, append-only repair for missing `ClanWarParticipation` rows. It uses existing `SyncCycle` plus conservative canonical `ClanPointsSync`/`ClanWarHistory` ownership and persisted `WarLookup` evidence; use `--apply` only after reviewing the bounded plan. It never creates SyncCycles or writes snapshots, Home rows, counters, or external data.
...
- Includes `/accounts` and `/link list` rep-badge rendering from persisted `TrackedClanRep` + `TrackedClan.clanBadge` data, placing the clan badge before the TH icon in `/accounts` and replacing the linked checkmark in `/link list`. `/link list` also supports a derived, read-only `Tenure` column: `C` counts canonical syncs observed on the active Home clan FWA war roster, `S` counts consecutive canonical syncs observed in the same FWA clan, and `A` counts consecutive canonical syncs observed in any tracked-alliance FWA clan. Pending roster evidence leaves existing values unchanged; tenure counters are not persisted, temporary Away periods do not reset C, lower bounds use `+`, and no active Home leaves C unavailable when physical streak evidence exists. `visibility:public` remains a shared, channel-visible roster where any server member can use the controls.
...
- `/clan configure` can persist an optional clan-owned designated `chat-channel`, and `/clan list tag:<tracked-clan>` inspects one tracked FWA clan's complete persisted configuration without external API calls.
...
- `/layout link:<link> [title] [description] [image] [img-url]` is the generic tracked-layout poster: it derives Town Hall through the shared Clash parser, reuses exact-link `LayoutRecord` lifecycle/post ownership, and publishes a minimal canonical public post with a private acknowledgement. `/fwa layout` remains the exclusive FWA `(Townhall, Type)` catalog management/view surface and keeps its transitional `FwaLayouts` compatibility copies synchronized.
- `/fwa match` and `/fwa mail send` share active-war mail freshness gating and only treat same-war, same-outcome references as up to date. Match parity is shown as unknown until exact same-war points evidence or unambiguous evidence-backed active-cycle evidence is available; a complete exact-war same-sync persisted projection may supply the FWA WIN/LOSE outcome for Send Mail eligibility, but never validated points or a bypass of match-type, identity, permission, or final-confirmation gates. Unequal points still resolve normally while equal-point tie-breaks wait for parity. `/fwa match copy_paste:true` posts the compact copy view directly without buttons, and `/fwa match copy_paste:true checklist:true visibility:public` posts the reaction-driven checklist version.
- When the points site is stale or unavailable, `/fwa match` retains each independently attributed resolver or already-loaded exact-war stored balance: complete safe balances are labeled Estimated/Persisted, unresolved finite balances identify their history limitation and baseline anchor, and stored fallbacks are labeled Last known/diagnostic. Diagnostic or partial balances never produce a winner, enable Send Mail, or become persisted current points.
- Mail and Bases checklists can render safely scoped inferred active match state immediately from persisted same-war evidence; inferred rows end with `⚠️` until `/fwa match` confirmation, while inactive/no-identity clans retain the skipped state.
- Manual Mail/Bases checklist refresh keeps the public `Refresh` control enabled, bounds preparation and the user-visible write wait to 30 seconds, and serializes concurrent refreshes per guild/message while allowing other checklist posts to proceed. Mail completion is owned by persisted tracked-message metadata and changes only from matching user reaction events; refreshes do not infer completion from reaction-cache presence.
...
- `/bot-logs type:clan-goals` configures live-war, canonical war-end, and scheduled-sync clan-goal notification routing with clan-log, clan-lead, clan-chat, generic bot-log, custom, or disabled destinations; `enable:clan-chat channel` resolves each clan's destination from its persisted `TrackedClan.chatChannelId`, skips clans without one, and never falls back to another route. It defaults to disabled and only uses `channel` for custom routing. Goal notifications remain non-pinging. Scheduled-sync zero-deviation eligibility is captured from immutable ACTUAL readiness snapshots at the sync boundary and delivered with sync-scoped idempotency.
...
- FWA stats and operations commands include canonical persisted `/fwa weight-health` reads from `FwaClanCatalog.weightSubmitDate` populated by the FWAStats `Clans.json` feed, plus its toggleable Weight Submission Zones view and `/fwa weight-alert` per-clan configuration, compliance checks, and layout management. The health view keeps the operational 7/30-day buckets; the zones view is a separate derived 4/6-week FWA timing/zone view, not an authoritative admin assignment. These commands no longer scrape the HTML weight page or require cookie authentication; the legacy age command and `/fwa weight-cookie` were removed, while `/fwa weight-link` remains unchanged. Enabled weight-alert policies are evaluated after fresh catalog syncs and use durable per-clan/date delivery state; routing remains on `TrackedClan` and mirror/staging runtimes do not send.
