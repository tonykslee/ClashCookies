...
- FWA stats and operations commands include canonical persisted `/fwa weight-health` reads from `FwaClanCatalog.weightSubmitDate` populated by the FWAStats `Clans.json` feed, plus `/fwa weight-alert` per-clan configuration, compliance checks, and layout management. These commands no longer scrape the HTML weight page or require cookie authentication; the legacy age command and `/fwa weight-cookie` were removed, while `/fwa weight-link` remains unchanged. Weight-alert configuration does not deliver alerts yet.
