...
- `/fwa base-swap clan:<tag> [war-bases:<positions>] [base-errors:<groups>] [fwa-bases:<positions>] [swap-reminder:<true|false>] [ping_role:<role>]` posts the tracked base-swap acknowledgement flow. `base-errors` can group positions with an optional note after the leading positions; commas separate groups, so notes cannot include commas, and the same note can apply to multiple positions in one group. The post keeps the usual TH layout links, reminder section, and DM copy lines.
