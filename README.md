...
- `/bot-logs type:clan-goals` configures live clan-goal notification routing with clan-log, clan-lead, generic bot-log, custom, or disabled destinations; it defaults to disabled and only uses `channel` for custom routing. Live war goals use the active war poll and remain non-pinging.
