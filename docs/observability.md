...
The existing `fwa_match_checklist_auto_post_scheduler` also evaluates current-sync Mail and Bases checklist posts for a durable 15-minute refresh cadence. It records the attempt, completion, and six-hour safety-stop state in checklist-owned tracked-message metadata, so restarts do not replay a due interval. Bounded `[fwa-checklist-auto-refresh]` events include `guild`, `sync_identity`, `view_type`, `message_id`, `result`, `reason`, `unresolved_count`, and `changed`; `refreshed`, `unchanged`, `completed`, `skipped`, and `failed` outcomes are emitted without message bodies. Mirror and staging runtimes do not refresh, and an Unknown Message target is marked deleted so it is not retried.
