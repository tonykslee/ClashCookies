...
- membership tenure analytics emits a bounded `[membership-streak] event=bulk_resolution` summary for guild/player/boundary counts plus one `[membership-streak] event=historical_canonicalization` aggregate with validated/dirty point, exact-tuple recovery, stale-ID, unmatched/ambiguous tuple, and canonical-history counts; active-war roster identity/read failures are WARN-level and fail closed as UNKNOWN, with no player-level roster payloads logged.
