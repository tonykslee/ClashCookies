...
- `HomeMembershipAnalyticsService` is a DB-first, read-only bulk combiner for `/link list` Home analytics. It reads active `ClanHomeMembershipPeriod` rows through `ClanHomeMembershipService` and uses one `MembershipStreakService` batch to derive non-persisted C/S/A values from the canonical boundary universe. C is credited to the current Home period, temporary Away/physical moves do not reset it, `+` preserves lower bounds, and no active Home is unavailable.
