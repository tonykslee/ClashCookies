...
- `MembershipStreakService` is a DB-first, read-only bulk consumer for derived clan/alliance streak analytics. It unions canonical `SyncCycle` boundaries with newer readiness/member snapshot boundaries, uses exact `SyncClanMemberSnapshot` evidence before historical fallback, and never writes calculated counters or creates a new state owner.
