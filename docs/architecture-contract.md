...
`ClanWarParticipation` is the canonical complete ended-war own-clan roster. For each canonical ended war, it contains one row per distinct participating player, including players with zero attacks; its distinct `playerTag` count is the roster/team size. It is not merely an attack-participation table. Historical membership positives and negatives may use this roster.

The normal war-end path establishes this invariant from `war.clan.members`: `WarAttacks` receives one `attackOrder=0` row per distinct member, and `persistWarParticipationSnapshot` builds one `ClanWarParticipation` row per distinct member from those rows. `WarLookup` is archived/raw evidence and reconstruction input, not the canonical completeness denominator. Its `teamSize` and `teamSizeSource` metadata remain compatibility/diagnostic fields only; they are not ownership or roster-size authority. The one-time participation backfill is all-or-nothing per canonical war and may append only when the full canonical participant roster is safely reconstructable.
...
`SyncClanMemberSnapshot` is the sole owner of immutable scheduled-sync snapshots of the persisted clan-member state available to that flow. It records the normalized rows captured for a scheduled boundary and intentionally does not claim a fresh physical observation at the scheduled second, nor store weight, composition, filler, or presentation fields. It remains the evidence source for current Home establishment policy, but it is not C/S/A authority: tenure uses canonical FWA war rosters. `AllianceClanMembershipInterval` separately represents continuously observed alliance/CWL membership over an observation interval. The active runtime writes both owners; `MirrorSyncService` only copies their persisted rows to staging and performs no new membership polling or writes.
...
- `SyncCycle` is the canonical guild-scoped mapping from a positive canonical sync number to the exact persisted scheduled sync time selected from the bounded prior schedule window. An exact or safely derived active FWA cycle may be persisted before war end using `ACTIVE_WAR_CONFIRMED`; inferred active FWA resolution remains a non-persisted candidate until explicit confirmation. Ended FWA history remains an idempotent corroborating path. `ClanWarHistory.prepStartTime` is not an exact SyncCycle time and a ScheduledSyncPost alone is not proof that a realized FWA cycle occurred. SyncCycle does not own war, readiness, filler, or message lifecycle state.
...
- `PointsEstimateResolverService` is a read-only, DB-first consumer for internal active-war point estimates. It does not own points, alter `ClanPointsSync`, allocate or persist `SyncCycle`, call points.fwafarm, or pass through `PointsDirectFetchGateService`. `/fwa match` may consume a complete, projection-safe pair on the same resolved active sync as visibly labeled display-only persisted/projected evidence while the website is stale; the resolver's `isEstimate` provenance flag does not remove that display-only boundary. Persisted/projected values never become validated website evidence or current-war/points-sync points writes. A complete, exact-war, same-sync, projection-safe pair may supply only an inferred FWA outcome for mail send eligibility; it never becomes validated website points and never bypasses inferred-match-type, manual-confirmation, identity, permission, or final-send gates. Checklist/mail parity remains a separate flow.
- FWA match rendering may retain each participant independently: a complete projection-safe balance is labeled Estimated/Persisted only when its resolved active sync is established, a finite `last_known_unresolved_history` balance identifies its history limitation and baseline anchor, and an exact-war stored fallback is labeled Last known/diagnostic. Partial or diagnostic values are display-only and cannot be combined into an outcome, mail authorization, validated evidence, or a points write.
- Estimate precedence is exact validated same-war `ClanPointsSync` evidence for the requested tag, then a correctly attributed validated same-war opponent balance, then a last-known persisted observation followed by each completed canonical `ClanWarHistory` delta in chronological order. Current evidence is validated only when its row sync matches the already resolved active sync and its war identity is exact; a conflicting row may remain historical evidence but is never current-matchup confirmation. Future-dated rows, rows beyond the active sync boundary, and history before the selected baseline are excluded. An observed baseline is classified from independently corroborating balance/result evidence, not fetch time alone: only a correctly identified observation successfully fetched before canonical war end, with matching sync evidence, can serve as the pre-war anchor for applying that war's result. A post-war observation is not adjusted again only when its post-war balance is independently established; otherwise an apparent post-war value remains diagnostic and unresolved. A non-null `pointsAfterWar` is usable only with independently verified result evidence and, when rule inputs permit, agreement with the calculated endpoint; the history row does not independently prove that numeric checkpoint, and disagreement is unresolved. The game-war identity in `ClanPointsSync.warId` is correlated to canonical history by exact start time and normalized clan/counterpart identities; it is never compared to the auto-incrementing `ClanWarHistory.warId` (which remains the `WarLookup` relationship key). Opponent observations use the opponent clan's own canonical history and never borrow the tracked clan's delta. FWA deltas require only independently verified actual results, MM deltas remain the shared zero rule, and BL retains its complete-input gates. A missing canonical actual result, required rule input, unverified checkpoint, missing pre-war provenance, or contiguous sync identity stops reconstruction and returns the last safe observed/derived value; an unresolved baseline is terminal and later wars must not be applied to it. An `actualOutcome` that is only an expected-outcome fallback is not independently verified, and it never treats `expectedOutcome` as an actual result or crosses an unresolved history gap as certain.
- The resolver exposes typed projection coverage: `complete_reconstruction` is safe for an active-war projection, `last_known_unresolved_history` retains a diagnostic balance but is not safe to present as today's balance, and `no_usable_balance` returns `balance: null`. Battle League deltas use the shared point rules only when their inputs are complete; an incomplete BL endpoint remains unsafe unless an independently supported balance anchor establishes it. After the applicable baseline is selected, reads are scoped to the requested clan and correctly attributed counterpart, with history bounded from that baseline and archived team-size reads limited to rows that require them; the resolver remains read-only.
- Estimates and stale-site persisted fallbacks are explicitly distinct from confirmed current-matchup evidence. A delayed points-site page may leave the public matchup gate unchanged while this internal consumer remains useful from persisted history; unavailable evidence remains `balance: null`.
...
- `SyncClanMemberSnapshot` is the immutable, per-guild/per-scheduled-sync/per-clan/player snapshot of persisted clan-member state available to the scheduled-sync flow. It remains the evidence source for Home establishment policy, but is not a fresh/exact-at-the-second physical observation and is not C/S/A evidence. It is intentionally separate from `AllianceClanMembershipInterval`, which remains authoritative for continuously observed alliance/CWL membership intervals. A player can validly have facts in multiple source clan contexts at one boundary; consumers must handle that ambiguity rather than collapsing the source facts.
...
- `SyncRetrospectiveService` is a DB-first, read-only consumer of `SyncCycle`, `ClanWarHistory`, `ClanWarParticipation`, persisted compliance, and mapped readiness snapshots. Ended-war participation coverage is complete when the canonical participation roster is non-empty and valid; it does not consult `WarLookup` team-size metadata. It preserves unknown coverage rather than converting incomplete evidence to zero and must not infer filler history from mutable registries or membership intervals.
- Tenure analytics authority: C/S/A observations are derived only from canonical-sync FWA war rosters—active/current `WarAttacks` attack-order-zero rows after shared active-war identity resolution, and ended-war `ClanWarParticipation` joined through guild-scoped canonical `ClanWarHistory`. `SyncClanMemberSnapshot`, readiness snapshots, alliance intervals, and feed-backed current-roster tables do not establish, override, or veto these observations.
- The following legacy descriptions of `MembershipStreakService`, `HomeMembershipAnalyticsService`, and `ClanWarParticipation` are superseded by the tenure-authority rule above: snapshots and alliance intervals are not C/S/A evidence, and participation is the canonical ended-war roster observation rather than a last-resort snapshot fallback.
- `MembershipStreakService` is a DB-first, read-only bulk consumer for derived clan/alliance streak analytics. Its C/S/A evidence axis is only database-distinct canonical `SyncCycle` boundaries. Active observations come from the shared current-war identity and `WarAttacks.attackOrder = 0` roster rows; ended observations come from guild-scoped canonical `ClanWarHistory` plus `ClanWarParticipation`. Unknown or ambiguous roster coverage is preserved, adjacent loaded boundaries require consecutive canonical `SyncCycle.syncNumber` identities, and sparse or contradictory boundaries are never bridged. Persisted, player-specific CWL roster/interval and event-owned round-lifecycle timing proof may mark an otherwise UNKNOWN or ABSENT existing boundary continuity-neutral for S/A; an unresolved CWL end is bounded only by that exact event's persisted round coverage, never by registry, event-instance, clan, fetch, or source-observation timestamps. Pre-CWL queueing may use an interval that continues into the resolved start without requiring event observation before the boundary. It never changes FWA status, increments a streak, bypasses canonical gaps, or affects Home ownership. It never writes calculated counters or creates a new state owner.
- `HomeMembershipAnalyticsService` is a DB-first, read-only bulk combiner for `/link list` Home analytics. It reads active `ClanHomeMembershipPeriod` rows through `ClanHomeMembershipService` and uses one `MembershipStreakService` batch to derive non-persisted C/S/A values from canonical-sync FWA roster observations. C is the nonconsecutive count of resolved canonical FWA-roster observations in the current Home clan across available canonical history; the Home period selects the Home clan but its `startedAtSyncTime` is not the historical C floor. S remains the physical current-clan streak and A remains the recognized-alliance streak, independent of Home. Temporary Away/physical moves do not reset C, unknown historical coverage produces a lower-bound C rather than guessed membership, `+` preserves per-metric lower bounds, and pending newest roster coverage preserves prior display values without inventing a new observation. A proven CWL-neutral latest UNKNOWN/ABSENT boundary keeps an anchored S/A displayable without making that boundary FWA evidence or changing C.
...
- `ClanWarParticipation` is the canonical ended-war FWA roster observation for C/S/A tenure analytics. It is admitted only through unambiguous guild-scoped canonical ownership from `ClanWarHistory`; incomplete or ambiguous participation remains UNKNOWN and is never backfilled into or written as `SyncClanMemberSnapshot`.
- The one-time membership-participation repair may append missing `ClanWarParticipation` rows from safely matched archived `WarLookup` canonical evidence. It never creates C/S/A counters, `SyncClanMemberSnapshot`, Home rows, or `SyncCycle` rows; unrecoverable or incomplete evidence remains UNKNOWN.
...
- FWA Mail/Bases checklist automatic refresh remains inside `FwaMatchChecklistAutoPostSchedulerService`: it resolves only active current-sync `TrackedMessage` checklist rows, persists cadence/terminal markers in checklist-owned metadata, and delegates in-place reconciliation to `TrackedMessageService`. Scheduler cycles use an explicit background CoC queue context. A Mail baseline may be reused only for unavailable live lookup evidence after exact CurrentWar context-key and same-sync identity validation; successful live responses, including verified not-in-war transitions, remain authoritative. It does not create a second scheduler, repost messages, or change publication claims.
