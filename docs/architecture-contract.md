...
`ClanHomeTransferCandidate` owns proposed Home transfers and their leader decisions only. It never owns current Home, Away, or Present state: a pending candidate does not change `ClanHomeMembershipPeriod`, and only explicit confirmation ends the old period and creates the successor Home period. Keep Home preserves the active Home and suppresses overlapping evidence until three fresh post-decision boundaries qualify. Transfer evidence must be three consecutive ordinary exact `SYNC_SNAPSHOT` boundaries in one different permanent `TrackedClan`; fallback participation, alliance intervals, filler state, and CWL boundaries cannot qualify. The shared persisted CWL-window primitive derived from `CwlTrackedClan`, event rows, and round/prep/history timing is the suppression authority. Production Home reconciliation writes candidates and decisions; mirror mode only copies the history.
...
- `MembershipStreakService` is a DB-first, read-only bulk consumer for derived clan/alliance streak analytics. It unions database-distinct canonical `SyncCycle`, readiness, and member-snapshot boundaries, uses exact `SyncClanMemberSnapshot` coverage before historical fallback (including authoritative physical absence), and never writes calculated counters or creates a new state owner.
...
- `HomeRosterService` is the read-only leadership read model for active Home periods. It counts only active `ClanHomeMembershipPeriod` rows for reserved slots, derives Present/Away from `FwaFeedSyncState(feedType=CLAN_MEMBERS, scopeType=CLAN_TAG).lastSuccessAt` plus `FwaClanMemberCurrent`, preserves Unknown when no successful current-roster coverage exists, and reports current physical occupants that have no Home assignment as diagnostic unassigned accounts. It bulk-enriches Away locations from authoritative persisted `PlayerCurrent` evidence, uses deterministic DB name fallbacks, and annotates pending `ClanHomeTransferCandidate` rows without mutating them. Present/Away/Unknown is never persisted.
