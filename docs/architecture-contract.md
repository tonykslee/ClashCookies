...
- `PointsEstimateResolverService` is a read-only, DB-first consumer for internal active-war point estimates. It does not own points, alter `ClanPointsSync`, allocate or persist `SyncCycle`, call points.fwafarm, or pass through `PointsDirectFetchGateService`. `/fwa match` may consume a complete, projection-safe pair on the same resolved active sync as visibly labeled display-only persisted/projected evidence while the website is stale; the resolver's `isEstimate` provenance flag does not remove that display-only boundary. Persisted/projected values never become validated website evidence, current-war/points-sync writes, or mail authorization, and checklist/mail parity remains a separate flow.
