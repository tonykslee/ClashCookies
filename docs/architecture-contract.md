...
- Estimate precedence is exact validated same-war `ClanPointsSync` evidence for the requested tag, then a correctly attributed validated same-war opponent balance, then a last-known persisted observation followed by each completed canonical `ClanWarHistory` delta in chronological order. Current evidence is validated only when its row sync matches the already resolved active sync and its war identity is exact; a conflicting row may remain historical evidence but is never current-matchup confirmation. Future-dated rows, rows beyond the active sync boundary, and history before the selected baseline are excluded. A missing canonical actual result, required rule input, or contiguous sync identity stops reconstruction and returns the last safe observed/derived value; it never treats `expectedOutcome` as an actual result or crosses an unresolved history gap as certain.
- The resolver exposes typed projection coverage: `complete_reconstruction` is safe for an active-war projection, `last_known_unresolved_history` retains a diagnostic balance but is not safe to present as today's balance, and `no_usable_balance` returns `balance: null`. Battle League deltas use the shared point rules only when their inputs are complete; a safely persisted `pointsAfterWar` checkpoint may be used when inputs are incomplete. Reads are scoped to the requested clan and correctly attributed counterpart, with bounded pre-active history queries; the resolver remains read-only.
