...
- FWA match rendering may retain each resolver participant independently: a complete projection-safe balance is labeled Estimated/Persisted, a finite `last_known_unresolved_history` balance is labeled Last known/history incomplete, and an unavailable participant remains unavailable. Partial or diagnostic values are display-only and cannot be combined into an outcome, mail authorization, validated evidence, or a points write.
