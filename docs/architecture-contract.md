...
- `MembershipStreakService` is a DB-first, read-only bulk consumer for derived clan/alliance streak analytics. Its C/S/A evidence axis is only database-distinct canonical `SyncCycle` boundaries. Active observations come from the shared current-war identity and `WarAttacks.attackOrder = 0` roster rows; ended observations come from guild-scoped canonical `ClanWarHistory` plus `ClanWarParticipation`. Unknown or ambiguous roster coverage is preserved, adjacent loaded boundaries require consecutive canonical `SyncCycle.syncNumber` identities, and sparse or contradictory boundaries are never bridged. Persisted, player-specific CWL roster/interval/timing proof may mark an otherwise UNKNOWN or ABSENT existing boundary continuity-neutral for S/A; with an unresolved CWL end, the matching persisted event's `lastObservedAt` must cover the exact boundary. Pre-CWL queueing may use an interval that continues into the resolved start without requiring event observation before the boundary. It never changes FWA status, increments a streak, bypasses canonical gaps, or affects Home ownership. It never writes calculated counters or creates a new state owner.
