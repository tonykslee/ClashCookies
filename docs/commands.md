...
- `/warplan set clan-tag:<tag> match-type:<FWA|BL|MM> plan-text:<text> [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Save custom war plan text for one clan. For `FWA`, omitting `outcome` sets `WIN` and both `LOSE` lose-style plans. For `FWA outcome:LOSE`, `lose-style` is required. Supports `{opponent}` placeholder.
- `/warplan show clan-tag:<tag> [match-type:<FWA|BL|MM>] [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Show effective plans for one clan. With no match-type, shows BL/MM plus `FWA-WIN` and both `FWA-LOSE` lose-style plans.
- `/warplan reset clan-tag:<tag> [match-type:<FWA|BL|MM>] [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Reset custom plans for one clan. With no match-type, resets all plans for that clan. For `FWA outcome:LOSE`, `lose-style` targets one LOSE style.
