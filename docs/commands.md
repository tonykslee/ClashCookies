...
- `/link list clan-tag:<clanTag>` - Render an embed roster with non-zero `Linked Users` / `Unlinked users` count buckets and padded inline-code rows (`TH | Player | ServerDisplayName` when member is resolvable, otherwise persisted linked username fallback, or `Unknown User`; unlinked rows show `#PLAYER_TAG`), with tracked-clan dropdown switching and deterministic ordering.
