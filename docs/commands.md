...
- `/link list clan-tag:<clanTag>` - Render an embed roster with non-zero `Linked Users` / `Unlinked users` count buckets and padded inline-code rows (`TH | Player | <@discordUserId>` or `TH | Player | #PLAYER_TAG`), with tracked-clan dropdown switching and deterministic ordering.
