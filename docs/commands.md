...
- `/fwa base-swap clan:<tag> [war-bases:<positions>] [base-errors:<positions>]` - Post a tracked base-swap acknowledgment message for the selected roster positions. The post now includes deduped TH layout lines using `RISINGDAWN` links from `FwaLayouts`, rendered as `## <a:arrow_arrow:1480819809338921142> TH{n} Link: <https://...>` between the player sections and the final react prompt.
