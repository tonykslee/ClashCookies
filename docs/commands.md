...
- `/warplan set match-type:<FWA|BL|MM> plan-text:<text> [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Save custom war plan text. For `FWA`, omitting `outcome` sets `WIN` and both `LOSE` lose-style plans. For `FWA outcome:LOSE`, `lose-style` is required. Supports `{opponent}` placeholder.
- `/warplan show [match-type:<FWA|BL|MM>] [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Show effective plans. With no args, shows BL/MM plus `FWA-WIN` and both `FWA-LOSE` lose-style plans.
- `/warplan reset [match-type:<FWA|BL|MM>] [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Reset custom plans to defaults. With no args, resets all plans. For `FWA outcome:LOSE`, `lose-style` targets one LOSE style.
