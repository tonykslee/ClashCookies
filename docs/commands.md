...
- `/warplan set clan-tag:<tag> match-type:<FWA|BL|MM> plan-text:<text> [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Save clan custom war plan text. `clan-tag` supports autocomplete from tracked clans. For `FWA`, omitting `outcome` sets `WIN` and both `LOSE` lose-style plans. For `FWA outcome:LOSE`, `lose-style` is required. Supports `{opponent}` placeholder.
...
- `/warplan set-default match-type:<FWA|BL|MM> plan-text:<text> [outcome:<WIN|LOSE>] [lose-style:<TRIPLE_TOP_30|TRADITIONAL>]` - Set editable guild default plan text used when a clan has no custom plan.
