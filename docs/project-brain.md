...
- Tenure roster authority: `SyncCycle + active-war identity/current persisted identity + WarAttacks(attackOrder=0) while an FWA war is active, or ClanWarHistory + ClanWarParticipation after archival -> MembershipStreakService`; C/S/A are canonical-sync war-roster statistics. `SyncClanMemberSnapshot`, readiness snapshots, alliance intervals, and feed-backed current-roster tables are not tenure evidence, and incomplete newest coverage remains pending/UNKNOWN. The one-time participation repair may append only safely reconstructed ended-war rows from matched archived `WarLookup` evidence; it never creates SyncCycles, counters, snapshots, or Home rows.
