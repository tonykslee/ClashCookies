...
- Tenure roster authority: `SyncCycle + active-war identity/current persisted identity + WarAttacks(attackOrder=0) while an FWA war is active, or ClanWarHistory + ClanWarParticipation after archival -> MembershipStreakService`; C/S/A are canonical-sync war-roster statistics. `ClanWarParticipation` is the complete canonical ended-war own-clan roster: one row per distinct canonical participant, including zero-attack players, with its distinct player count defining roster size. `WarLookup` is archived/raw reconstruction evidence; `teamSize` metadata is compatibility/diagnostic data rather than the completeness denominator. `SyncClanMemberSnapshot`, readiness snapshots, alliance intervals, and feed-backed current-roster tables are not tenure evidence, and incomplete newest coverage remains pending/UNKNOWN. The one-time participation repair is all-or-nothing per canonical war and may append only a safely reconstructed full roster; it never creates SyncCycles, counters, snapshots, or Home rows.
- Home tenure analytics: `ClanHomeMembershipPeriod + MembershipStreakService batch -> HomeMembershipAnalyticsService -> /link list Tenure`; C counts only resolved roster observations in the active Home clan after Home start, while S/A are consecutive canonical-sync roster observations and pending newest coverage preserves prior displayed values.
- Read-only membership streak analytics: `SyncCycle + shared active-war identity + WarAttacks(attackOrder=0) / ClanWarHistory + ClanWarParticipation -> MembershipStreakService`; this is a bounded bulk DB-first reader for canonical-sync FWA physical clan streaks and monitored-alliance streaks. Active rows are used only after exact persisted identity resolution; ended participation is joined through canonical guild-scoped history; UNKNOWN and ambiguous coverage remain unresolved; adjacent streak boundaries require consecutive canonical `SyncCycle.syncNumber` values; and no derived streak counters are persisted.
- Read-only Home membership analytics: `ClanHomeMembershipPeriod + MembershipStreakService batch -> HomeMembershipAnalyticsService -> /link list Tenure`; C counts only resolved FWA roster observations matching the active Home clan from the Home start, S is the authoritative consecutive streak in the current physical clan, and A is the authoritative monitored-alliance streak. C requires an active Home, while S/A remain independently available when physical evidence is safely resolved. Pending newest coverage preserves prior displayed values, counters are derived only, temporary Away/physical moves do not reset C, and lower bounds use `+`.
