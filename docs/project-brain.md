...
- Snapshot-backed todo: `PlayerLink + TodoUserUsage + CurrentWar + CurrentCwlRound/CwlRoundMemberCurrent + activity signals -> TodoSnapshotService -> TodoPlayerSnapshot`, with current membership stored separately from WAR lineup identity so stale WAR context can be cleared independently of the latest clan observation.
