...
- Internal points estimate: `ClanPointsSync + ClanWarHistory + read-only SyncCycle chronology -> PointsEstimateResolverService`; this is a DB-first read model, not a second points owner. It first uses validated same-war evidence, then correctly attributed opponent observations, then reconstructs from an observed/derived baseline and completed-war deltas using the shared FWA/MM/BL rules. Estimates remain labeled as estimates and are intentionally available while the website still displays a previous matchup; missing actual results, required rule inputs, or contiguous history stop reconstruction without inventing a balance.
