...
- Roster signup foundation: `RosterService -> Roster / RosterGroup / RosterSignup`, with `visitorSignupOpensAt` now persisted as nullable roster metadata for later signup-window enforcement and roster-post display work. It is not enforced or rendered yet. `RosterService` also owns a pure delayed-signup eligibility policy that consumes already-resolved `visitorSignupOpensAt`, configured delayed roles, alliance-member precedence, manager bypass, and the exact `now < visitorSignupOpensAt` / `now >= visitorSignupOpensAt` boundary. Discord role discovery and signup interaction enforcement remain pending.
