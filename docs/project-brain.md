...
- Internal points estimate: `ClanPointsSync + ClanWarHistory + read-only SyncCycle chronology -> PointsEstimateResolverService`; this is a DB-first read model, not a second points owner. It first uses validated same-war evidence only when the row sync matches the resolved active sync and the active-war identity is exact, then correctly attributed opponent observations, then reconstructs from an observed/derived baseline and completed-war deltas using the shared FWA/MM/BL rules. Persisted observation/end-time evidence distinguishes pre-war baselines, whose own result is applied once, from post-war checkpoints, which are not adjusted again; opponent observations use the opponent clan's own history. Typed coverage distinguishes complete reconstruction (projection-safe), a retained last-known balance with unresolved history (diagnostic only), and no usable balance. Future/beyond-boundary rows, pre-baseline history, missing actual results, incomplete BL inputs, or contiguous-history gaps never become a confirmed current projection; a safe persisted `pointsAfterWar` checkpoint may cover incomplete BL inputs. Reads are clan-scoped and bounded from the selected baseline, archived team-size payloads are read only when needed, and the resolver performs no writes or points-site work.
