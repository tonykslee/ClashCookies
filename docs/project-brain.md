...
- Durable Home membership: `SyncClanMemberSnapshot + SyncClanReadinessSnapshot historical filler facts + TrackedClan -> ClanHomeMembershipService -> ClanHomeMembershipPeriod`; active production reconciliation establishes an account/player Home only after three consecutive exact non-filler boundaries in one permanent tracked clan. Home periods are guild-scoped, one-active-period constrained, never silently transferred or ended by this automation, and are mirrored read-only to staging.
