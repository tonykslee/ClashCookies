...
- Snapshot-backed todo: `PlayerLink + TodoUserUsage + CurrentWar + CurrentCwlRound/CwlRoundMemberCurrent + activity signals -> TodoSnapshotService -> TodoPlayerSnapshot`, with current membership stored separately from WAR lineup identity and event-owned RAID/CWL/GAMES context so stale event state can be cleared independently of the latest clan observation.
