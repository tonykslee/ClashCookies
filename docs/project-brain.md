...
- Read-only Home membership analytics: `ClanHomeMembershipPeriod -> HomeMembershipAnalyticsService + MembershipStreakService batch -> /link list Tenure`; C counts canonical boundaries from the current active Home start, S is the existing physical streak interpreted against that Home clan, and A is the existing monitored-alliance streak. Counters are derived only, temporary Away/physical moves do not reset C, lower bounds use `+`, and no active Home is shown as unavailable.
