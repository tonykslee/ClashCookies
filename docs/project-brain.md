...
- FWA match points display: `/fwa match` consumes the two independent resolver results once per stale matchup. It keeps complete safe balances visibly labeled Estimated/Persisted, retains finite `last_known_unresolved_history` balances as Last known/history incomplete, and marks each missing side independently; only the existing complete exact-war same-sync safe pair can derive an outcome or satisfy the mail outcome gate.
