...
- Roster signup foundation: `RosterService -> Roster / RosterGroup / RosterSignup`, with `visitorSignupOpensAt` now persisted as nullable roster metadata for later signup-window enforcement and roster-post display work. The public `Signup` panel now enforces the delayed visitor window before it opens account selection, while final confirmation revalidation remains pending. `RosterService` also owns a pure delayed-signup eligibility policy that consumes already-resolved `visitorSignupOpensAt`, configured delayed roles, alliance-member precedence, manager bypass, and the exact `now < visitorSignupOpensAt` / `now >= visitorSignupOpensAt` boundary. Discord/config resolution failures for the public gate fail open with bounded logging.
- Autorole delayed-signup config: `AutoRoleGuildConfig -> delayedSignupRoleIds`, with multiple Discord roles persisted for later delayed-signup policy use. It is configurable through Discord commands now, but it is still not enforced by roster signup logic, and `nonMemberRoleId` remains the separate existing autorole control.
...
| Autorole delayed-signup role IDs | AutoRoleGuildConfig.delayedSignupRoleIds |
