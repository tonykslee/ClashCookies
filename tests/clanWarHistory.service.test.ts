...
describe("ClanWarHistoryService", () => {
  ...
  it("queries ended wars by exact sync numbers without stretching the range", async () => {
    const findMany = vi.fn().mockResolvedValue([]);
    const service = new ClanWarHistoryService({ clanWarHistory: { findMany } } as any);

    await service.listEndedByClanSyncNumbers({
      clanTag: "AAA111",
      syncNumbers: [516, 517, 545],
    });
