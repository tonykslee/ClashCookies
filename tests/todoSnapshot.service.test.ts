...
import {
  buildPreloadedLiveCurrentWarRosterIndexByPlayerTag,
...
function installMutableTodoSnapshotStore(rows: Record<string, unknown>[]): void {
  prismaMock.todoPlayerSnapshot.findMany.mockImplementation(async () =>
    rows.map((row) => ({ ...row })),
  );
  txMock.todoPlayerSnapshot.findUnique.mockImplementation(async (args: any) => {
    const playerTag = String(args?.where?.playerTag ?? "");
    const row = rows.find((candidate) => candidate.playerTag === playerTag);
    return row ? { ...row } : null;
  });
  txMock.todoPlayerSnapshot.upsert.mockImplementation(async (args: any) => {
    const playerTag = String(args?.where?.playerTag ?? "");
    const nextRow = {
      ...(rows.find((candidate) => candidate.playerTag === playerTag) ?? {}),
      ...(args?.create ?? {}),
      ...(args?.update ?? {}),
    };
    const index = rows.findIndex((candidate) => candidate.playerTag === playerTag);
    if (index >= 0) {
      rows[index] = nextRow;
    } else {
      rows.push(nextRow);
...
describe("TodoSnapshotService", () => {
  beforeEach(() => {
    ...
    prismaMock.todoPlayerSnapshot.findUnique.mockResolvedValue(null);
    ...
    txMock.todoPlayerSnapshot.upsert = prismaMock.todoPlayerSnapshot.upsert;
    ...
    prismaMock.playerCurrent.findMany.mockResolvedValue([
      {
        playerTag: playerTagA,
        playerName: "attk for me",
        currentClanTag: "#YLJV2R0R",
        currentClanName: "Elsewhere",
        lastSeenAt: new Date("2026-03-26T00:10:00.000Z"),
        lastFetchedAt: new Date("2026-03-26T00:10:00.000Z"),
        lastSource: "manual",
    ...
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
      {
        ...
        sourceUpdatedAt: new Date("2026-03-26T00:01:00.000Z"),
        sourceWarId: 1000538,
        sourceWarStartTime: new Date("2026-03-25T12:00:00.000Z"),
        sourceWarEndTime: new Date("2026-03-26T12:00:00.000Z"),
        sourceWarState: "inWar",
        sourceCurrentWarUpdatedAt: new Date("2026-03-26T00:01:00.000Z"),
  ...
  it("tracks ambiguous live current-war players for tracked refresh without choosing a generic owner", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const zeroGravityClanTag = "#LQQ99UV8";
    const playerTag = "#QGRJ2222";
    const currentWarStartTime = new Date("2026-03-25T12:00:00.000Z");
    const refreshSpy = vi
      .spyOn(todoSnapshotService as any, "refreshSnapshotsForPlayerTagsInternal")
      .mockResolvedValue({ playerCount: 1, updatedCount: 1 } as any);
    const preloadedCurrentWarSnapshotsByClanTag = new Map([
      [
        rockyRoadClanTag,
        {
          state: "preparation",
    ...
    prismaMock.playerLink.findMany.mockResolvedValue([
      { discordUserId: "143827744717799425", playerTag },
    ]);
    prismaMock.todoUserUsage.findMany.mockResolvedValue([
      { discordUserId: "143827744717799425" },
    ]);
    ...
    prismaMock.trackedClan.findMany.mockResolvedValue([
      { tag: rockyRoadClanTag, name: "Rocky Road" },
      { tag: zeroGravityClanTag, name: "ZERO GRAVITY" },
    ]);
    prismaMock.raidTrackedClan.findMany.mockResolvedValue([]);
    prismaMock.cwlTrackedClan.findMany.mockResolvedValue([]);
    prismaMock.cwlPlayerClanSeason.findMany.mockResolvedValue([]);
    ...
    try {
      const rosterIndex = buildPreloadedLiveCurrentWarRosterIndexByPlayerTag({
        clanTags: [rockyRoadClanTag, zeroGravityClanTag],
        preloadedCurrentWarSnapshotsByClanTag,
      });
      expect(rosterIndex.byPlayerTag.has(playerTag)).toBe(false);
      expect(rosterIndex.ambiguousPlayerTags.has(playerTag)).toBe(true);
      ...
      expect(result.nonTrackedPlayerCount).toBe(0);
      expect(refreshSpy).toHaveBeenCalledTimes(1);
      expect(refreshSpy.mock.calls[0]?.[0]?.playerTags).toEqual([playerTag]);
    } finally {
      refreshSpy.mockRestore();
    }
  });

  it("keeps an ambiguous live WAR player unresolved instead of selecting Rocky Road or ZERO GRAVITY", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const zeroGravityClanTag = "#LQQ99UV8";
    const playerTag = "#QGRJ2222";
    const currentWarStartTime = new Date("2026-03-25T12:00:00.000Z");
    ...
    try {
      ...
      expect(warUpdate.warClanTag).toBeNull();
      expect(warUpdate.warClanName).toBeNull();
      expect(warUpdate.warPosition).toBeNull();
      expect(warUpdate.warOwnerSource).toBe("NONE");
      expect(warUpdate.warOwnerWarId).toBeNull();
      expect(warUpdate.warOwnerVerifiedAt).toBeNull();
      expect(warUpdate.warActive).toBe(false);
      expect(warUpdate.warAttacksUsed).toBe(0);
      expect(warUpdate.warAttacksMax).toBe(2);
      expect(warUpdate.warPhase).toBeNull();
      expect(warUpdate.warEndsAt).toBeNull();
      expect(
        getSingleConsoleMessage(
          consoleInfoSpy,
          (message) => message.includes("event=todo_war_owner_resolution_summary"),
        ),
      ).toContain("ambiguous_live_match_count=1");
      expect(
        getSingleConsoleMessage(
          consoleInfoSpy,
          (message) => message.includes("event=todo_war_owner_resolution_summary"),
        ),
      ).toContain("unresolved_count=0");
    } finally {
      consoleInfoSpy.mockRestore();
    }
  });

  it("falls back to unresolved canonical ownership when exact candidates tie without a selected candidate", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const playerTag = "#QGRJ2222";
    const currentWarStartTime = new Date("2026-03-25T12:00:00.000Z");
    const consoleInfoSpy = vi.spyOn(console, "info").mockImplementation(() => undefined);
    ...
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
      {
        ...
        sourceUpdatedAt: new Date("2026-03-26T00:01:00.000Z"),
        sourceWarId: 1000548,
        sourceWarStartTime: currentWarStartTime,
        sourceWarEndTime: new Date("2026-03-26T12:00:00.000Z"),
        sourceWarState: "inWar",
        sourceCurrentWarUpdatedAt: new Date("2026-03-26T00:01:00.000Z"),
        updatedAt: new Date("2026-03-26T00:01:00.000Z"),
    ...
    try {
      ...
      ).toContain("unresolved_canonical_count=1");
    } finally {
      consoleInfoSpy.mockRestore();
    }
  });

  it("keeps a third live-clan observation ambiguous without reintroducing the player into byPlayerTag", () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const zeroGravityClanTag = "#LQQ99UV8";
    ...
    const rosterIndex = buildPreloadedLiveCurrentWarRosterIndexByPlayerTag({
      clanTags: [rockyRoadClanTag, zeroGravityClanTag, twcClanTag],
      preloadedCurrentWarSnapshotsByClanTag,
    });

    expect(rosterIndex.byPlayerTag.has(playerTag)).toBe(false);
    expect(rosterIndex.ambiguousPlayerTags.has(playerTag)).toBe(true);
    expect(rosterIndex.indexedMemberCount).toBe(3);
  });
  ...
  it("corrects a stale Rocky Road WAR owner to TheWiseCowboys when live lineup proves the player belongs there", async () => {
    ...
    try {
      ...
      expect(cocService.getCurrentWar).not.toHaveBeenCalled();
      const upsertCall = prismaMock.todoPlayerSnapshot.upsert.mock.calls[0]?.[0] as
        | {
      ...
      const persistedWar = (upsertCall?.update ?? upsertCall?.create) as Record<string, unknown>;
      expect(persistedWar).toMatchObject({
        clanTag: twcClanTag,
        clanName: "TheWiseCowboys",
  ...
  it("keeps a verified TWC WAR owner when a stale Rocky Road refresh completes out of order", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    ...
    expect(cocService.getCurrentWar).toHaveBeenCalled();
  ...
  it("lets a canonical tracked-roster refresh replace an older verified Rocky Road owner when no fresh live response is available", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#G89J8LCVU";
    const olderVerifiedAt = new Date("2026-03-26T00:01:00.000Z");
    ...
    const existingRow = buildSnapshotRow({
      ...
      warOwnerSource: "LIVE_VERIFIED",
      warOwnerWarId: 1001,
      warOwnerVerifiedAt: olderVerifiedAt,
      warPosition: 18,
      warAttacksUsed: 1,
      warSourceUpdatedAt: olderVerifiedAt,
      clanMembershipObservedAt: new Date("2026-03-26T00:00:00.000Z"),
    });

    prismaMock.todoPlayerSnapshot.findMany.mockResolvedValue([existingRow]);
    txMock.todoPlayerSnapshot.findUnique.mockResolvedValue(existingRow);
    prismaMock.playerCurrent.findMany.mockResolvedValue([]);
    prismaMock.fwaClanMemberCurrent.findMany.mockResolvedValue([
      {
        playerTag,
    ...
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
      { clanTag: rockyRoadClanTag },
      {
        ...
        sourceWarEndTime: new Date("2026-03-26T12:00:00.000Z"),
        sourceWarState: "inWar",
        sourceCurrentWarUpdatedAt: new Date("2026-03-26T00:01:30.000Z"),
      },
    ]);
    prismaMock.fwaTrackedClanWarRosterMemberCurrent.findMany.mockResolvedValue([
      {
    ...
    prismaMock.currentWar.findMany.mockResolvedValue([
      ...
      },
      {
        ...
        startTime: new Date("2026-03-25T12:00:00.000Z"),
        endTime: new Date("2026-03-26T12:00:00.000Z"),
        updatedAt: new Date("2026-03-26T00:01:30.000Z"),
      },
    ]);
    prismaMock.trackedClan.findMany.mockResolvedValue([
      { tag: rockyRoadClanTag, name: "Rocky Road" },
  ...
  it("keeps a newer live verified owner when a delayed canonical tracked-roster refresh arrives later", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#G89J8LCVU";
    const newerVerifiedAt = new Date("2026-03-26T00:03:00.000Z");
    const delayedAttemptAt = new Date("2026-03-26T00:02:00.000Z");
    ...
    const existingRow = buildSnapshotRow({
      ...
      warPosition: 18,
      warAttacksUsed: 1,
      warSourceUpdatedAt: newerVerifiedAt,
      clanMembershipObservedAt: new Date("2026-03-26T00:00:00.000Z"),
    });
    ...
    prismaMock.fwaClanMemberCurrent.findMany.mockResolvedValue([
      {
        ...
        clanTag: rockyRoadClanTag,
        playerName: "Party Blizzard",
        sourceSyncedAt: delayedAttemptAt,
      },
    ]);
    prismaMock.fwaWarMemberCurrent.findMany.mockResolvedValue([]);
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
    ...
    await todoSnapshotService.refreshSnapshotsForPlayerTags({
      playerTags: [playerTag],
      cocService: cocService as any,
      nowMs: delayedAttemptAt.getTime(),
    });
  ...
  it("repairs same-identity canonical roster metadata without downgrading live provenance", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#G89J8LCVU";
    const existingVerifiedAt = new Date("2026-03-26T00:01:00.000Z");
    const attemptedAt = new Date("2026-03-26T00:02:00.000Z");
    ...
    const existingRow = buildSnapshotRow({
      ...
      warOwnerSource: "LIVE_VERIFIED",
      warOwnerWarId: 1002,
      warOwnerVerifiedAt: existingVerifiedAt,
      warPosition: 18,
      warAttacksUsed: 2,
      warAttacksMax: 2,
      warPhase: "battle day",
      warEndsAt,
      warSourceUpdatedAt: existingVerifiedAt,
      clanMembershipObservedAt: new Date("2026-03-26T00:00:00.000Z"),
    });
    ...
    expect(prismaMock.todoPlayerSnapshot.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        update: expect.objectContaining({
          ...
          warClanName: "TheWiseCowboys",
          warPosition: 22,
          warSourceUpdatedAt: new Date("2026-03-26T00:01:30.000Z"),
          warOwnerSource: "LIVE_VERIFIED",
          warOwnerWarId: 1002,
          warOwnerVerifiedAt: existingVerifiedAt,
          warActive: true,
  ...
  it("keeps a newer live verified owner when a same-identity canonical refresh arrives stale", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#G89J8LCVU";
    const existingVerifiedAt = new Date("2026-03-26T00:03:00.000Z");
    const attemptedAt = new Date("2026-03-26T00:02:00.000Z");
    const warEndsAt = new Date("2026-03-26T12:00:00.000Z");
  ...
  it("lets an older live-verified war state fall back when the current war ID changes", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const playerTag = "#G89J8LCVU";
    const existingVerifiedAt = new Date("2026-03-26T00:01:00.000Z");
    const attemptedAt = new Date("2026-03-26T00:02:00.000Z");
  ...
  it("keeps matching current-war live continuity protected against an ordinary fallback", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const playerTag = "#G89J8LCVU";
    const verifiedAt = new Date("2026-03-26T00:01:00.000Z");
    const attemptedAt = new Date("2026-03-26T00:02:00.000Z");
    const warEndsAt = new Date("2026-03-26T12:00:00.000Z");
  ...
  it("moves planned acceptance suppression to transaction-time when a newer live row is present at write time", async () => {
    const consoleInfoSpy = vi.spyOn(console, "info").mockImplementation(() => undefined);
    ...
    const existingVerifiedAt = new Date("2026-03-26T00:03:00.000Z");
    const warEndsAt = new Date("2026-03-26T12:00:00.000Z");
    const planningRows: Record<string, unknown>[] = [
      buildSnapshotRow({
        playerTag,
        playerName: "Party Blizzard",
        clanTag: rockyRoadClanTag,
    ...
    const cocService = {
      getPlayerRaw: vi.fn().mockResolvedValue(null),
      getCurrentWar: vi.fn().mockResolvedValue(liveCurrentWar),
    };
  ...
  it("allows a planned suppression to resolve to an accepted transaction-time write", async () => {
    const consoleInfoSpy = vi.spyOn(console, "info").mockImplementation(() => undefined);
    const consoleWarnSpy = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    const rockyRoadClanTag = "#2RYGLU2UY";
    const playerTag = "#G89J8LCVU";
    ...
    const planningRows: Record<string, unknown>[] = [
      buildSnapshotRow({
        ...
        warOwnerSource: "LIVE_VERIFIED",
        warOwnerWarId: 1001,
        warOwnerVerifiedAt: new Date("2026-03-26T00:03:00.000Z"),
        warPosition: 18,
        warAttacksUsed: 1,
        warAttacksMax: 2,
        warPhase: "battle day",
        warEndsAt,
        warSourceUpdatedAt: new Date("2026-03-26T00:03:00.000Z"),
        clanMembershipObservedAt: new Date("2026-03-26T00:00:00.000Z"),
      }),
    ];
    const transactionRows: Record<string, unknown>[] = [];
  ...
  it("emits the ownership summary only after the snapshot writes complete", async () => {
    const consoleInfoSpy = vi.spyOn(console, "info").mockImplementation(() => undefined);
    const consoleWarnSpy = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    const rockyRoadClanTag = "#2RYGLU2UY";
    const playerTag = "#G89J8LCVU";
    ...
    try {
      ...
      const summaryIndex = consoleInfoSpy.mock.calls.findIndex(([message]) =>
        String(message).includes("event=todo_war_owner_resolution_summary"),
      );
      expect(summaryIndex).toBeGreaterThanOrEqual(0);
      const summaryCallOrder = consoleInfoSpy.mock.invocationCallOrder[summaryIndex] ?? 0;
      const writeCallOrder = prismaMock.todoPlayerSnapshot.upsert.mock.invocationCallOrder[0] ?? 0;
      expect(summaryCallOrder).toBeGreaterThan(writeCallOrder);
    } finally {
      consoleInfoSpy.mockRestore();
      consoleWarnSpy.mockRestore();
    }
  ...
  it("reports persist_failed and skips the final summary when the write transaction rejects", async () => {
    const consoleInfoSpy = vi.spyOn(console, "info").mockImplementation(() => undefined);
    const consoleWarnSpy = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    ...
    installMutableTodoSnapshotStore(transactionRows);
    prismaMock.todoPlayerSnapshot.findMany.mockImplementation(async () => []);
    txMock.todoPlayerSnapshot.upsert.mockRejectedValueOnce(new Error("boom"));
    prismaMock.playerCurrent.findMany.mockResolvedValue([]);
    prismaMock.fwaClanMemberCurrent.findMany.mockResolvedValue([]);
    prismaMock.fwaWarMemberCurrent.findMany.mockResolvedValue([
      {
    ...
    try {
      ...
      expect(
        consoleErrorSpy.mock.calls.some(
          ([message]) => String(message).includes("persist_failed"),
        ),
      ).toBe(true);
      expect(
        consoleInfoSpy.mock.calls.some((call) =>
          String(call[0] ?? "").includes("event=todo_war_owner_resolution_summary"),
        ),
      ).toBe(false);
    } finally {
      consoleInfoSpy.mockRestore();
  ...
  it("chooses a different applicable clan when the exact TWC response excludes the player but Rocky Road confirms them", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#G89J8LCVU";
    const verifiedAt = new Date("2026-03-26T00:02:00.000Z");
  ...
  it("keeps an exact inactive TWC candidate out of the way so Rocky Road generic evidence can win", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
  ...
  it("keeps an exact retained-ended TWC candidate attached to the same ended war", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
  ...
  it("updates exact retained-ended attack progress without changing retained ownership", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
  ...
  it("keeps retained-ended attack progress when mismatched ended WarAttacks rows arrive", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    ...
    prismaMock.warAttacks.findMany.mockResolvedValue([
      {
        ...
        defenderPosition: 7,
        stars: 3,
        attackSeenAt: new Date("2026-03-24T00:10:00.000Z"),
      },
    ]);
    prismaMock.trackedClan.findMany.mockResolvedValue([
      { tag: twcClanTag, name: "TheWiseCowboys" },
  ...
  it("authoritatively clears a verified Rocky Road WAR owner when the live lineup excludes the player", async () => {
    ...
    await todoSnapshotService.refreshSnapshotsForPlayerTags({
      ...
      nowMs: Date.UTC(2026, 2, 26, 0, 4, 0, 0),
    });
  ...
  it("lets the later Rocky Road refresh replace a prior TWC live owner when the active war changes", async () => {
    ...
    const twcCurrentWar = {
      ...
      startTime: new Date("2026-03-25T12:00:00.000Z"),
      endTime: new Date("2026-03-26T12:00:00.000Z"),
      updatedAt: new Date("2026-03-26T00:02:00.000Z"),
    };
    const rockyRoadCurrentWar = {
      clanTag: rockyRoadClanTag,
      warId: 1001,
      state: "inWar",
      startTime: new Date("2026-03-25T12:00:00.000Z"),
    ...
    let currentWarRows = [twcCurrentWar];
    let trackedWarRosterRows = [
      {
        clanTag: twcClanTag,
        playerTag,
        position: 8,
    ...
    prismaMock.currentWar.findMany.mockImplementation(async () => currentWarRows.map((row) => ({ ...row })));
    prismaMock.fwaTrackedClanWarRosterMemberCurrent.findMany.mockImplementation(async () =>
      trackedWarRosterRows.map((row) => ({ ...row })),
    );
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
      { clanTag: rockyRoadClanTag },
      { clanTag: twcClanTag },
    ...
    await todoSnapshotService.refreshSnapshotsForPlayerTags({
      ...
      nowMs: Date.UTC(2026, 2, 26, 0, 2, 0, 0),
    });

    currentWarRows = [rockyRoadCurrentWar];
  ...
  it.each([
    {
      label: "older",
      existingVerifiedAt: new Date("2026-03-26T00:02:00.000Z"),
      clearObservationAt: new Date("2026-03-26T00:00:00.000Z"),
      shouldClear: false,
    },
    {
      label: "newer",
      existingVerifiedAt: new Date("2026-03-26T00:00:00.000Z"),
      clearObservationAt: new Date("2026-03-26T00:02:00.000Z"),
      shouldClear: true,
    },
  ])(
    "handles $label authoritative clear against a verified TWC owner",
    async ({ existingVerifiedAt, clearObservationAt, shouldClear }) => {
      const twcClanTag = "#29PCQGUV0";
      const playerTag = "#PYLQ0289";
      ...
      await todoSnapshotService.refreshSnapshotsForPlayerTags({
        ...
        nowMs: clearObservationAt.getTime(),
      });
  ...
  it("keeps newer live-verified TWC attacks and position when an older same-owner refresh arrives stale", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    const verifiedAt = new Date("2026-03-26T00:02:00.000Z");
  ...
  it("keeps an active bootstrap TWC snapshot stable while the current war remains active", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    ...
    installMutableTodoSnapshotStore(rows);
    prismaMock.playerCurrent.findMany.mockResolvedValue([]);
    prismaMock.fwaClanMemberCurrent.findMany.mockResolvedValue([
      {
        playerTag,
  ...
  it("clears an expired bootstrap TWC snapshot when the clan no longer has an active war", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    ...
    await todoSnapshotService.refreshSnapshotsForPlayerTags({
      playerTags: [playerTag],
      cocService: cocService as any,
      nowMs: Date.UTC(2026, 2, 26, 0, 6, 0, 0),
    });
  ...
  it("authoritatively clears an active bootstrap TWC snapshot when the live lineup excludes the player", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
  ...
  it("upgrades an active bootstrap TWC snapshot to live verification when the live lineup contains the player", async () => {
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    const verifiedAt = new Date("2026-03-26T00:02:00.000Z");
  ...
  it("lets an expired bootstrap TWC snapshot yield to Rocky Road fallback resolution", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
  ...
  it("clears WAR context when a single derived Rocky Road candidate is disproven by preload", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const playerTag = "#PYLQ0289";
    const currentWarStartTime = new Date("2026-03-25T12:00:00.000Z");
  ...
  it("keeps a stale raw Rocky Road WAR member candidate behind the live TWC lineup", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    const currentWarStartTime = new Date("2026-03-25T12:00:00.000Z");
  ...
  it("keeps the ambiguous live WAR match deterministic regardless of persisted candidate order", async () => {
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const playerTag = "#PYLQ0289";
    const currentWarStartTime = new Date("2026-03-25T12:00:00.000Z");
  ...
  it("falls back to the raw feed attack count when no exact WarAttacks row matches", async () => {
    prismaMock.fwaClanMemberCurrent.findMany.mockResolvedValue([
      {
        playerTag: "#PYLQ0289",
        clanTag: "#PQL0289",
  ...
  it("rejects previous-war WarAttacks rows for the current tracked war", async () => {
    prismaMock.fwaClanMemberCurrent.findMany.mockResolvedValue([
      {
        playerTag: "#PYLQ0289",
        clanTag: "#PQL0289",
  ...
  it("prefers a tracked roster row when warId matches even if the start time differs", async () => {
    prismaMock.playerLink.findMany.mockResolvedValue([]);
    prismaMock.todoUserUsage.findMany.mockResolvedValue([]);
    prismaMock.todoPlayerSnapshot.findMany.mockResolvedValue([]);
    prismaMock.playerCurrent.findMany.mockResolvedValue([]);
    ...
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
      {
        clanTag: "#29PCQGUV0",
        sourceWarId: 1002,
        sourceWarStartTime: new Date("2026-03-24T12:00:00.000Z"),
        sourceWarEndTime: new Date("2026-03-24T13:00:00.000Z"),
        sourceWarState: "inWar",
        sourceCurrentWarUpdatedAt: new Date("2026-03-24T00:00:00.000Z"),
      },
    ]);
    prismaMock.fwaTrackedClanWarRosterMemberCurrent.findMany.mockResolvedValue([
      {
    ...
    prismaMock.trackedClan.findMany.mockResolvedValue([
      { tag: "#29PCQGUV0", name: "TheWiseCowboys" },
    ]);
    prismaMock.raidTrackedClan.findMany.mockResolvedValue([]);
    prismaMock.cwlTrackedClan.findMany.mockResolvedValue([]);
    prismaMock.currentCwlRound.findMany.mockResolvedValue([]);
  ...
  it("accepts a tracked roster row by exact war start time when warId is absent", async () => {
    prismaMock.playerLink.findMany.mockResolvedValue([]);
    prismaMock.todoUserUsage.findMany.mockResolvedValue([]);
    prismaMock.todoPlayerSnapshot.findMany.mockResolvedValue([]);
    prismaMock.playerCurrent.findMany.mockResolvedValue([]);
    ...
    await todoSnapshotService.refreshSnapshotsForPlayerTags({
      playerTags: ["#PYLQ0289"],
      nowMs: Date.UTC(2026, 2, 25, 12, 1, 0, 0),
    });
  ...
  it("prefers the freshest tracked parent when multiple exact WAR identities match the same player", async () => {
    const playerTag = "#PYLQ0288";
    const rockyRoadClanTag = "#2RYGLU2UY";
    const twcClanTag = "#29PCQGUV0";
    const newerUpdatedAt = new Date("2026-03-26T00:02:00.000Z");
    ...
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
      {
        ...
        sourceWarState: "inWar",
        sourceCurrentWarUpdatedAt: new Date("2026-03-26T00:00:00.000Z"),
        sourceUpdatedAt: new Date("2026-03-25T23:58:00.000Z"),
        observedAt: new Date("2026-03-25T23:59:00.000Z"),
      },
      {
        clanTag: twcClanTag,
        clanName: "TheWiseCowboys",
        ...
        sourceWarEndTime: new Date("2026-03-26T12:00:00.000Z"),
        sourceWarState: "inWar",
        sourceCurrentWarUpdatedAt: newerUpdatedAt,
        sourceUpdatedAt: newerUpdatedAt,
        observedAt: newerUpdatedAt,
      },
    ]);
    prismaMock.fwaTrackedClanWarRosterMemberCurrent.findMany.mockResolvedValue([
      {
  ...
  it("rejects a stale tracked roster identity row so raw fallback can win", async () => {
    prismaMock.playerLink.findMany.mockResolvedValue([]);
    prismaMock.todoUserUsage.findMany.mockResolvedValue([]);
    prismaMock.todoPlayerSnapshot.findMany.mockResolvedValue([]);
    prismaMock.playerCurrent.findMany.mockResolvedValue([]);
    ...
    prismaMock.fwaTrackedClanWarRosterCurrent.findMany.mockResolvedValue([
      {
        clanTag: "#29PCQGUV0",
        sourceWarId: 1003,
        sourceWarStartTime: new Date("2026-03-20T12:00:00.000Z"),
        sourceWarEndTime: new Date("2026-03-20T13:00:00.000Z"),
        sourceWarState: "finished",
        sourceCurrentWarUpdatedAt: new Date("2026-03-20T00:00:00.000Z"),
      },
    ]);
    prismaMock.fwaTrackedClanWarRosterMemberCurrent.findMany.mockResolvedValue([
      {
