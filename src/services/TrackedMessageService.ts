...
export type FwaMatchChecklistTrackedRow = {
  ...
  /** Typed outcome used by automatic completion decisions; older rows may omit it. */
...
export type FwaMatchChecklistTrackedMetadata = {
  ...
  basesReactionBaselines?: FwaMatchChecklistReactionBaseline[];
  ...
  opponentTag?: string | null;
  warStartTimeIso?: string | null;
  /** Durable automatic-refresh cadence state. */
  expectedTrackedClanTags?: string[];
  autoRefreshLastAttemptAtIso?: string | null;
  autoRefreshCompletedAtIso?: string | null;
  autoRefreshStoppedAtIso?: string | null;
  autoRefreshClaimToken?: string | null;
};

export type FwaMatchChecklistReactionBaseline = {
  contextKey: string;
  reactionKey: string;
...
function mergeFwaMatchChecklistMetadataAfterConflict(params: {
  ...
  for (const key of keys) {
    ...
    if (key === "checkedClanTags") {
      ...
      const baseTags = normalizeTags(params.base.checkedClanTags);
      const desiredTags = normalizeTags(params.desired.checkedClanTags);
      const latestTags = normalizeTags(params.latest.checkedClanTags);
      const mergedTags = new Set<string>();
      for (const clanTag of new Set([...baseTags, ...desiredTags, ...latestTags])) {
        const desiredChanged = desiredTags.has(clanTag) !== baseTags.has(clanTag);
        const latestChanged = latestTags.has(clanTag) !== baseTags.has(clanTag);
        const checked = latestChanged
          ? latestTags.has(clanTag)
          : desiredChanged
            ? desiredTags.has(clanTag)
            : baseTags.has(clanTag);
        if (checked) mergedTags.add(clanTag);
      }
      merged.checkedClanTags = [...mergedTags];
      continue;
    }
...
export async function findLatestFwaMatchChecklistCheckedClanTags(params: {
  ...
  for (const row of rows) {
    ...
    if (
      ...
      (!syncIdentity || rowSyncIdentity === syncIdentity)
    ) {
...
function parseFwaMatchChecklistRow(value: unknown): FwaMatchChecklistTrackedRow | null {
  ...
  return {
    ...
    outcome:
      ...
      String(value.outcome ?? "").trim().toUpperCase() === "LOSE" ||
      String(value.outcome ?? "").trim().toUpperCase() === "UNKNOWN"
        ? (String(value.outcome).trim().toUpperCase() as "WIN" | "LOSE" | "UNKNOWN")
        : null,
...
export function parseFwaMatchChecklistMetadata(
  ...
  const checkedClanTags = Array.isArray(value.checkedClanTags)
    ...
    : [];
  const basesReactionBaselines = Array.isArray(value.basesReactionBaselines)
    ? [...new Map(
        value.basesReactionBaselines
          .map((baseline) => parseFwaMatchChecklistReactionBaseline(baseline))
          .filter((baseline): baseline is FwaMatchChecklistReactionBaseline => Boolean(baseline))
          .map((baseline) => [buildFwaMatchChecklistReactionBaselineKey(baseline), baseline]),
      ).values()]
    : [];
  ...
  return {
    ...
    expectedTrackedClanTags: Array.isArray(value.expectedTrackedClanTags)
      ? [
          ...new Set(
            value.expectedTrackedClanTags
              .map((clanTag) => normalizeChecklistClanTag(String(clanTag ?? "")))
              .filter((clanTag): clanTag is string => Boolean(clanTag)),
          ),
        ]
      : undefined,
    autoRefreshLastAttemptAtIso: String(value.autoRefreshLastAttemptAtIso ?? "").trim() || null,
    autoRefreshCompletedAtIso: String(value.autoRefreshCompletedAtIso ?? "").trim() || null,
    autoRefreshStoppedAtIso: String(value.autoRefreshStoppedAtIso ?? "").trim() || null,
    autoRefreshClaimToken: String(value.autoRefreshClaimToken ?? "").trim() || null,
    rows,
  };
}

/** Purpose: decide automatic-refresh completion from typed row state, never rendered Discord text. */
export function assessFwaMatchChecklistCompletion(params: {
  rows: FwaMatchChecklistTrackedRow[];
  expectedTrackedClanTags?: string[] | null;
}): { complete: boolean; unresolvedCount: number; reason: string } {
  const expected = [
    ...new Set(
      (params.expectedTrackedClanTags ?? [])
        .map((clanTag) => normalizeChecklistClanTag(clanTag))
        .filter((clanTag): clanTag is string => Boolean(clanTag)),
    ),
  ];
  if (expected.length === 0) {
    return { complete: false, unresolvedCount: 1, reason: "missing_expected_roster" };
  }

  const rowsByTag = new Map<string, FwaMatchChecklistTrackedRow>();
  for (const row of params.rows) {
    const clanTag = normalizeChecklistClanTag(row.clanTag);
    if (clanTag && !rowsByTag.has(clanTag)) rowsByTag.set(clanTag, row);
  }
  let unresolvedCount = 0;
  for (const clanTag of expected) {
    const row = rowsByTag.get(clanTag);
    if (!row) {
      unresolvedCount += 1;
      continue;
    }
    const opponentTag = normalizeChecklistClanTag(row.opponentTag ?? "");
    const hasActualOpponent = Boolean(
      opponentTag &&
        !["-", "?", "UNKNOWN", "NONE", "NULL", "N/A", "NA"].includes(opponentTag),
    );
    const warId = normalizeWarIdText(row.warId ?? null);
    const warStartTimeMs = row.warStartTimeIso ? Date.parse(row.warStartTimeIso) : NaN;
    const hasValidWarIdentity = Boolean(warId || Number.isFinite(warStartTimeMs));
    const matchType = String(row.matchType ?? "").trim().toUpperCase();
    const resolvedMatchType = matchType === "FWA" || matchType === "BL" || matchType === "MM";
    const outcome = String(row.outcome ?? "").trim().toUpperCase();
    const resolvedOutcome = outcome === "WIN" || outcome === "LOSE";
    const resolved =
      hasActualOpponent &&
      hasValidWarIdentity &&
      resolvedMatchType &&
      (matchType === "BL" || matchType === "MM" || resolvedOutcome);
    if (!resolved) unresolvedCount += 1;
  }
  const rowCoverageMatches =
    rowsByTag.size === expected.length && expected.every((clanTag) => rowsByTag.has(clanTag));
  if (!rowCoverageMatches) {
    unresolvedCount += Math.max(0, rowsByTag.size - expected.length);
  }
  return {
    complete: rowCoverageMatches && unresolvedCount === 0,
    unresolvedCount,
    reason: rowCoverageMatches ? "typed_rows_unresolved" : "incomplete_row_coverage",
  };
}

/** Purpose: compare render-relevant typed checklist state while ignoring reaction acknowledgments. */
export function areFwaMatchChecklistRowsEqual(
  currentRows: FwaMatchChecklistTrackedRow[],
  nextRows: FwaMatchChecklistTrackedRow[],
): boolean {
  if (currentRows.length !== nextRows.length) return false;
  const normalize = (row: FwaMatchChecklistTrackedRow) => ({
    clanTag: normalizeChecklistClanTag(row.clanTag),
    compactCopyLine: row.compactCopyLine,
    badgeEmojiId: row.badgeEmojiId ?? null,
    badgeEmojiName: row.badgeEmojiName ?? null,
    badgeEmojiInline: row.badgeEmojiInline ?? "",
    basesStatus: row.basesStatus ?? null,
    matchType: row.matchType ?? null,
    matchStateInferred: row.matchStateInferred === true,
    outcome: row.outcome ?? null,
    contextKey: row.contextKey ?? null,
    detailLines: row.detailLines ?? null,
    warId: normalizeWarIdText(row.warId ?? null),
    opponentTag: normalizeChecklistClanTag(row.opponentTag ?? "") || null,
    warStartTimeIso: row.warStartTimeIso ?? null,
  });
  return currentRows.every((row, index) =>
    JSON.stringify(normalize(row)) === JSON.stringify(normalize(nextRows[index])),
  );
}
...
function parseFwaMatchChecklistReactionBaseline(
  value: unknown,
): FwaMatchChecklistReactionBaseline | null {
  if (!isObject(value)) return null;
  const contextKey = String(value.contextKey ?? "").trim();
  const reactionKey = String(value.reactionKey ?? "").trim();
  const status = normalizeFwaMatchChecklistReactionBaselineStatus(value.status);
  if (!contextKey || !reactionKey) return null;
  if (status === "pending") {
    return {
      contextKey,
      reactionKey,
  ...
  const userCount = Math.max(0, Math.trunc(Number(value.userCount ?? 0)));
  return {
    contextKey,
    reactionKey,
    userCount,
...
function normalizeFwaMatchChecklistReactionBaselineStatus(
  value: unknown,
): "captured" | "pending" | null {
  const status = String(value ?? "").trim();
  if (status === "captured" || status === "pending") return status;
  return null;
}

function buildFwaMatchChecklistReactionKey(input: {
  emojiId: string | null;
  emojiName: string | null;
}): string | null {
  const emojiId = String(input.emojiId ?? "").trim();
  if (emojiId) return `custom:${emojiId}`;
  const emojiName = normalizeChecklistEmojiName(input.emojiName);
  if (!emojiName) return null;
  return `unicode:${emojiName}`;
}

function buildFwaMatchChecklistReactionBaselineKey(
  baseline: Pick<FwaMatchChecklistReactionBaseline, "contextKey" | "reactionKey">,
): string {
  return `${baseline.contextKey}::${baseline.reactionKey}`;
}

function getFwaMatchChecklistReactionUserCount(
  reaction: Pick<FwaMatchChecklistReactionCacheEntry, "count" | "me">,
): number {
  return Math.max(0, Math.trunc(Number(reaction.count ?? 0)) - (reaction.me === true ? 1 : 0));
}

function findFwaMatchChecklistReactionEntry(
  cache: FwaMatchChecklistReactionCache,
  row: Pick<FwaMatchChecklistTrackedRow, "badgeEmojiId" | "badgeEmojiName">,
): FwaMatchChecklistReactionCacheEntry | undefined {
  return [...cache.values()].find((candidate) =>
...
function findFwaMatchChecklistReactionBaseline(
  baselines: Iterable<FwaMatchChecklistReactionBaseline>,
  row: Pick<FwaMatchChecklistTrackedRow, "contextKey" | "badgeEmojiId" | "badgeEmojiName">,
): FwaMatchChecklistReactionBaseline | null {
  const contextKey = String(row.contextKey ?? "").trim();
  if (!contextKey) return null;
  const reactionKey = buildFwaMatchChecklistReactionKey({
    emojiId: row.badgeEmojiId,
    emojiName: row.badgeEmojiName,
  });
  if (!reactionKey) return null;
  const baselineKey = buildFwaMatchChecklistReactionBaselineKey({
    contextKey,
    reactionKey,
  });
  return [...baselines].find(
    (baseline) => buildFwaMatchChecklistReactionBaselineKey(baseline) === baselineKey,
  ) ?? null;
}

function buildFwaMatchChecklistReactionBaselinesByKey(
  baselines: Iterable<FwaMatchChecklistReactionBaseline>,
): Map<string, FwaMatchChecklistReactionBaseline> {
  return new Map(
    [...baselines].map((baseline) => [buildFwaMatchChecklistReactionBaselineKey(baseline), baseline]),
  );
}

function isFwaMatchChecklistReactionBaselinePending(
  baseline: Pick<FwaMatchChecklistReactionBaseline, "status"> | null | undefined,
): boolean {
  return baseline?.status === "pending";
}

function isFwaMatchChecklistReactionObservationAuthoritative(params: {
  reaction?: Pick<FwaMatchChecklistReactionCacheEntry, "me"> | null;
  observation: Pick<FwaChecklistReactionObservation, "fetchSucceeded">;
}): boolean {
  if (params.reaction) {
    return params.reaction.me !== undefined;
  }
  return params.observation.fetchSucceeded === true;
}

function collectFwaMatchChecklistReactionBaselines(params: {
  rows: FwaMatchChecklistTrackedRow[];
  ...
  const nextByKey = new Map<string, FwaMatchChecklistReactionBaseline>();
  for (const row of params.rows) {
    ...
    const previousRow = params.previousRowsByTag.get(normalizeChecklistClanTag(row.clanTag)) ?? null;
    const previousContextKey = String(previousRow?.contextKey ?? "").trim();
    const previousReactionKey = previousRow
      ? buildFwaMatchChecklistReactionKey({
          emojiId: previousRow.badgeEmojiId,
          emojiName: previousRow.badgeEmojiName,
        })
      : null;
    const shouldCapture =
      (previousRow?.basesStatus === "skipped" && row.basesStatus !== "skipped") ||
      (previousContextKey !== "" && previousContextKey !== contextKey && row.basesStatus !== "skipped") ||
      (previousReactionKey !== null && previousReactionKey !== reactionKey && row.basesStatus !== "skipped");
    const existing = existingByKey.get(baselineKey) ?? null;
    const reaction = findFwaMatchChecklistReactionEntry(params.reactionCache, row);
    if (isFwaMatchChecklistReactionBaselinePending(existing)) {
      ...
      if (existing) {
        nextByKey.set(baselineKey, existing);
        continue;
      }
      continue;
    }
...
function normalizeChecklistEmojiName(value: string | null | undefined): string {
  return String(value ?? "").trim().normalize("NFC");
}

type FwaMatchChecklistReconcileSource = "listener" | "button" | "scheduled";

type FwaMatchChecklistReconcileTarget = {
  clanTag: string;
  badgeEmojiInline: string;
  badgeEmojiId: string | null;
  badgeEmojiName: string | null;
...
/** Purpose: describe one final checklist row as a bot-managed badge reaction target. */
function buildFwaMatchChecklistReconcileTarget(
  row: Pick<
    FwaMatchChecklistTrackedRow,
    "clanTag" | "badgeEmojiInline" | "badgeEmojiId" | "badgeEmojiName" | "basesStatus"
  >,
  viewType: "Mail" | "Bases",
): FwaMatchChecklistReconcileTarget | null {
  if (!shouldApplyFwaMatchChecklistBadgeReaction(row, viewType)) return null;
  const badgeEmojiInline = String(row.badgeEmojiInline ?? "").trim();
  if (!badgeEmojiInline) return null;
  const customMatch = /^<a?:([A-Za-z0-9_]{2,32}):(\d{1,22})>$/.exec(badgeEmojiInline);
  if (customMatch) {
    return {
      clanTag: normalizeChecklistClanTag(row.clanTag),
      badgeEmojiInline,
      badgeEmojiId: customMatch[2] ?? null,
      badgeEmojiName: customMatch[1] ?? null,
      emojiType: "custom",
      reactionKey: `custom:${customMatch[2]}`,
    };
  }
  const badgeEmojiName = normalizeChecklistEmojiName(row.badgeEmojiName ?? badgeEmojiInline);
  if (!badgeEmojiName || !looksLikeUnicodeChecklistEmoji(badgeEmojiName)) return null;
  return {
    clanTag: normalizeChecklistClanTag(row.clanTag),
    badgeEmojiInline,
    badgeEmojiId: null,
    badgeEmojiName,
    emojiType: "unicode",
    reactionKey: `unicode:${badgeEmojiName}`,
  };
}

/** Purpose: accept only emoji-like Unicode checklist identifiers, not plain text placeholders. */
function looksLikeUnicodeChecklistEmoji(value: string): boolean {
  const normalized = normalizeChecklistEmojiName(value);
  if (!normalized) return false;
  if (/^<a?:/.test(normalized)) return false;
...
/** Purpose: reconcile bot-managed checklist badge reactions from the final rendered checklist rows. */
async function reconcileChecklistBadgeReactions(params: {
  guildId: string;
  messageId: string;
  message: {
    partial?: boolean;
  ...
  for (const row of params.rows) {
    if (!shouldApplyFwaMatchChecklistBadgeReaction(row, params.viewType)) continue;
    const target = buildFwaMatchChecklistReconcileTarget(row, params.viewType);
    if (!target) {
      failedCount += 1;
      console.warn(
  ...
  const uniqueTargets = [...new Map(targets.map((target) => [target.reactionKey, target])).values()];
  if (uniqueTargets.length === 0) return;

  const findMatchingReaction = (
    cache: FwaMatchChecklistReactionCache,
    target: FwaMatchChecklistReconcileTarget,
  ) =>
    [...cache.values()].find((candidate) =>
      emojiMatches(candidate, {
        emojiId: target.badgeEmojiId,
        emojiName: target.badgeEmojiName,
      }),
  ...
  for (const target of uniqueTargets) {
    const reaction = findMatchingReaction(reactionCache, target);
    const targetAuthoritative = reaction
      ? reaction.me !== undefined
      : reactionObservation.fetchSucceeded === true;
    if (!targetAuthoritative || reaction?.me === true) continue;
    if (typeof params.message.react !== "function") {
      failedCount += 1;
      console.warn(
        `[fwa-checklist] event=reaction_reconcile_failed guild=${params.guildId} message=${params.messageId} badge=${target.clanTag} emoji_type=${target.emojiType} failure=react_unavailable`,
      );
      continue;
    }
    try {
      await params.message.react(target.badgeEmojiInline);
      addedCount += 1;
    } catch (err) {
      failedCount += 1;
      console.warn(
        `[fwa-checklist] event=reaction_reconcile_failed guild=${params.guildId} message=${params.messageId} badge=${target.clanTag} emoji_type=${target.emojiType} failure=${summarizeChecklistReactionFailureCategory(err)}`,
      );
    }
  }
...
/** Purpose: summarize checklist reaction-add failures with bounded telemetry categories. */
function summarizeChecklistReactionFailureCategory(err: unknown): string {
  const code = (err as { code?: number } | null | undefined)?.code;
  if (code === 50013 || code === 50001) return "permission_denied";
  if (code === 10014) return "unknown_emoji";
  if (typeof code === "number") return `discord_error_${code}`;
  return "unknown_error";
}

export function shouldApplyFwaMatchChecklistBadgeReaction(
  row: Pick<FwaMatchChecklistTrackedRow, "badgeEmojiInline" | "basesStatus">,
...
export class TrackedMessageService {
  ...
  private async reconcileFwaChecklistMetadataAfterConflict(params: {
    ...
    const editAfterPersist = async (
      metadata: FwaMatchChecklistTrackedMetadata,
      phase: "initial_edit" | "reconciliation_edit",
      attempt?: number,
    ): Promise<boolean> => {
      recoveryExpectedContents.add(params.renderContent(params.baseMetadata));
      recoveryExpectedContents.add(params.renderContent(metadata));
      try {
        await params.message.edit({
          content: params.renderContent(metadata),
          allowedMentions: { parse: [] },
        });
        return true;
      } catch (err) {
        console.error(
          `[tracked-message] event=fwa_checklist_refresh_failed guild=${params.tracked.guildId} messageId=${params.tracked.messageId} phase=${phase}${attempt === undefined ? "" : ` attempt=${attempt}`} error=${formatError(err)}`,
        );
    ...
    // A newly confirmed completion must reach the CAS before Discord is edited.
    // Refresh-only row changes retain the established edit/reconcile ordering because
    // they do not create a new completion claim.
    const completionChanged = !checklistMetadataValueEqual(
      params.baseMetadata.checkedClanTags ?? [],
      params.desiredMetadata.checkedClanTags ?? [],
    );
    if (!completionChanged) {
      try {
        await params.message.edit({
          content: params.renderContent(params.desiredMetadata),
          allowedMentions: { parse: [] },
        });
      } catch (err) {
        console.error(
          `[tracked-message] event=fwa_checklist_refresh_failed guild=${params.tracked.guildId} messageId=${params.tracked.messageId} phase=initial_edit error=${formatError(err)}`,
        );
        throw err;
      }
    }
    ...
    for (let attempt = 1; attempt <= FWA_CHECKLIST_METADATA_RECONCILIATION_ATTEMPTS; attempt += 1) {
      ...
      const reconciledExpiresAt =
        params.extendedExpiresAt && latestExpiry
          ...
          : params.extendedExpiresAt ?? latestExpiry;
      if (!completionChanged) {
        recoveryExpectedContents.add(params.renderContent(mergedMetadata));
        try {
          await params.message.edit({
            content: params.renderContent(mergedMetadata),
            allowedMentions: { parse: [] },
          });
        } catch (err) {
          console.error(
      ...
      if (persisted.count === 1) {
        if (
          completionChanged &&
          !(await editAfterPersist(mergedMetadata, "reconciliation_edit", attempt))
        ) {
          return null;
        }
        console.info(
  ...
  async claimFwaMatchChecklistAutoRefresh(params: {
    messageId: string;
    nowMs: number;
    safetyCutoffAtMs: number;
    intervalMs: number;
  }): Promise<FwaMatchChecklistAutoRefreshClaim> {
    const tracked = await prisma.trackedMessage.findUnique({
      where: { messageId: params.messageId },
    });
    if (!tracked || tracked.status !== TRACKED_MESSAGE_STATUS.ACTIVE) {
      return { claimed: false, reason: "expired", claimToken: null, metadata: null };
    }
    if (!(tracked.expiresAt instanceof Date) || tracked.expiresAt.getTime() <= params.nowMs) {
      return { claimed: false, reason: "expired", claimToken: null, metadata: null };
    }
    const metadata = parseFwaMatchChecklistMetadata(tracked.metadata);
    if (!metadata) {
      return { claimed: false, reason: "missing_metadata", claimToken: null, metadata: null };
    }
    if (metadata.autoRefreshCompletedAtIso) {
      return { claimed: false, reason: "completed", claimToken: null, metadata };
    }
    if (metadata.autoRefreshStoppedAtIso) {
      return { claimed: false, reason: "stopped", claimToken: null, metadata };
    }
    if (params.nowMs >= params.safetyCutoffAtMs) {
      const stoppedAtIso = new Date(params.nowMs).toISOString();
      const stoppedMetadata: FwaMatchChecklistTrackedMetadata = {
        ...metadata,
        autoRefreshStoppedAtIso: stoppedAtIso,
        autoRefreshClaimToken: null,
      };
      await prisma.trackedMessage
        .updateMany({
          where: {
            id: tracked.id,
            status: TRACKED_MESSAGE_STATUS.ACTIVE,
          ...
          data: { metadata: stoppedMetadata as any },
        })
        .catch(() => undefined);
      return { claimed: false, reason: "stopped", claimToken: null, metadata };
    }
    const anchorMs = Date.parse(metadata.createdAtIso);
    if (!Number.isFinite(anchorMs)) {
      return { claimed: false, reason: "missing_metadata", claimToken: null, metadata };
    }
    const elapsedMs = params.nowMs - anchorMs;
    if (elapsedMs < params.intervalMs) {
      return { claimed: false, reason: "not_due", claimToken: null, metadata };
    }
    const cadenceSlotAtMs = anchorMs + Math.floor(elapsedMs / params.intervalMs) * params.intervalMs;
    const lastAttemptMs = metadata.autoRefreshLastAttemptAtIso
      ? Date.parse(metadata.autoRefreshLastAttemptAtIso)
      : NaN;
    if (Number.isFinite(lastAttemptMs) && lastAttemptMs >= cadenceSlotAtMs) {
      return { claimed: false, reason: "not_due", claimToken: null, metadata };
    }

    const claimToken = `fwa-checklist-auto-refresh:${params.messageId}:${params.nowMs}:${Math.random().toString(36).slice(2)}`;
    const nextMetadata: FwaMatchChecklistTrackedMetadata = {
      ...metadata,
      autoRefreshLastAttemptAtIso: new Date(params.nowMs).toISOString(),
      autoRefreshClaimToken: claimToken,
    };
    const claimed = await prisma.trackedMessage.updateMany({
      where: {
        id: tracked.id,
        status: TRACKED_MESSAGE_STATUS.ACTIVE,
        expiresAt: { gt: new Date(params.nowMs) },
        metadata: tracked.metadata as any,
      },
      data: { metadata: nextMetadata as any },
    });
    if (claimed.count !== 1) {
      return { claimed: false, reason: "concurrent_update", claimToken: null, metadata };
    }
    return { claimed: true, reason: "claimed", claimToken, metadata: nextMetadata };
  }

  /** Purpose: persist automatic-refresh completion/stop state only if this worker still owns its claim. */
  async finalizeFwaMatchChecklistAutoRefresh(params: {
    messageId: string;
    claimToken: string;
    expectedTrackedClanTags?: string[] | null;
    ...
    if (!tracked || tracked.status !== TRACKED_MESSAGE_STATUS.ACTIVE) return false;
    const metadata = parseFwaMatchChecklistMetadata(tracked.metadata);
    if (!metadata || metadata.autoRefreshClaimToken !== params.claimToken) return false;
    const nextMetadata: FwaMatchChecklistTrackedMetadata = {
      ...metadata,
      autoRefreshClaimToken: null,
      ...(params.expectedTrackedClanTags
        ...
        : {}),
      ...(params.completedAtIso ? { autoRefreshCompletedAtIso: params.completedAtIso } : {}),
      ...(params.stoppedAtIso ? { autoRefreshStoppedAtIso: params.stoppedAtIso } : {}),
    };
    const updated = await prisma.trackedMessage.updateMany({
      where: {
        id: tracked.id,
        status: TRACKED_MESSAGE_STATUS.ACTIVE,
        metadata: tracked.metadata as any,
  ...
  private async refreshFwaMatchChecklistMessageUnlocked(
    ...
    if (
      ...
      (!options.autoRefreshClaimToken ||
        metadata.autoRefreshClaimToken !== options.autoRefreshClaimToken)
    ) {
      return false;
    }

    const viewType = resolveFwaMatchChecklistViewType(tracked.metadata);
    const reconcileSource: FwaMatchChecklistReconcileSource = change
      ? "listener"
      : options?.automatic
        ? "scheduled"
        : options?.rows
    ...
    if (viewType === "Bases") {
      ...
      const existingReactionBaselines = metadata.basesReactionBaselines ?? [];
      const existingPendingReactionBaselineKeys = new Set(
        existingReactionBaselines
          .filter((baseline) => isFwaMatchChecklistReactionBaselinePending(baseline))
          .map((baseline) =>
      ...
      reactionObservation = await observeFwaMatchChecklistReactionCacheForRows({
        ...
        forceFetch: existingPendingReactionBaselineKeys.size > 0,
      });
      ...
      if (change && changedRowTag) {
        ...
        if (matchedRow && matchedRow.basesStatus !== "skipped") {
          handledChangeRowTags.add(changedRowTag);
          const matchedReaction = findFwaMatchChecklistReactionEntry(
            reactionCacheForObservation,
            matchedRow,
          );
          const matchedReactionCount = matchedReaction?.count ?? reactionChange.reaction.count ?? 0;
          const matchedReactionUserCount = matchedReaction
            ? getFwaMatchChecklistReactionUserCount(matchedReaction)
            : Math.max(0, Math.trunc(Number(matchedReactionCount ?? 0)));
          const matchedBaseline = findFwaMatchChecklistReactionBaseline(
            sourceTransitionBaselines,
            matchedRow,
          );
          if (reactionChange.kind === "add") {
            ...
            isFwaMatchChecklistReactionObservationAuthoritative({
              reaction: matchedReaction,
              observation: reactionObservation,
            })
          ) {
            if (isFwaMatchChecklistReactionBaselinePending(matchedBaseline)) {
              console.warn(
                `[fwa_checklist_reaction_observation] guildId=${tracked.guildId} messageId=${message.id} clanTag=${changedRowTag} reason=pending_baseline_unresolved`,
              );
            } else {
              const checked =
      ...
      for (const row of effectiveRows) {
        ...
        const baselineKey = baseline
          ...
          : null;
        if (baselineKey && existingPendingReactionBaselineKeys.has(baselineKey)) {
          continue;
        }
        if (
          !isFwaMatchChecklistReactionObservationAuthoritative({
        ...
        const shouldPersist = baseline
          ? baseline.userCount !== null && currentUserCount > baseline.userCount
          : (reaction.count ?? 0) > 1;
        if (shouldPersist) {
      ...
      const extendedExpiresAt = options?.automatic
        ? null
        : resolveExtendedChecklistExpiresAt(
    ...
    // Mail completion is durable event state. Reaction observation below is only
    // used by badge reconciliation; cache presence must not rewrite this set.
    ...
    if (change && changedRowTag) {
      ...
      if (matchedRow && reactionChange.kind === "add") {
        ...
        if (humanCount === 0) {
          ...
          console.warn(
            `[fwa_checklist_reaction_observation] guildId=${tracked.guildId} messageId=${message.id} clanTag=${changedRowTag} reason=source_observation_unavailable action=preserve_completion`,
          );
        }
      }
    }
