...
import { parseCocApiTime } from "../commands/fwa/dataParsers";
...
export function resolveActiveWarIdentityPatch(
  ...
  const liveOpponentName = String(input.liveWar?.opponent?.name ?? "").trim() || null;
  const liveClanName = String(input.liveWar?.clan?.name ?? "").trim() || null;
