...
type ChecklistPersistedSyncRow = PreparedStoredSyncMatchRow & {
  clanTag: string;
  warId: string | null;
  warStartTime: Date;
  outcome: string | null;
...
type ChecklistEffectiveMatchState = {
...
type ChecklistLiveWarIdentity = {
  state: "preparation" | "inWar" | "notInWar" | "unknown";
  warId: number | string | null;
  startTime: Date | null;
  opponentTag: string | null;
};

type CurrentWarLookupResult =
...
function resolveChecklistLiveWarIdentity(liveWar: {
  state?: string | null;
  warId?: number | string | null;
  startTime?: string | null;
  opponent?: { tag?: string | null } | null;
} | null): ChecklistLiveWarIdentity | null {
  if (!liveWar) return null;
  const startMs = parseCocApiTime(liveWar.startTime ?? null);
  return {
    state: normalizeWarState(liveWar.state ?? null),
    warId: liveWar.warId ?? null,
    startTime: startMs === null ? null : new Date(startMs),
    opponentTag: normalizeChecklistClanTag(liveWar.opponent?.tag ?? "") || null,
  };
}

function normalizeChecklistWarId(value: number | string | null | undefined): number | null {
...
async function resolveChecklistEffectiveMatchState(input: {
...
}): Promise<ChecklistEffectiveMatchState> {
  const currentMatchType = normalizeMatchType(input.currentWar?.matchType ?? null);
  const currentWarState = normalizeWarState(input.currentWar?.state ?? null);
  const hasSafeActiveWarIdentity = Boolean(
    input.activeWar &&
      normalizeChecklistClanTag(input.activeWar.opponentTag ?? "") &&
      ((input.activeWar.warId !== null &&
        input.activeWar.warId !== undefined &&
        Number.isFinite(Number(input.activeWar.warId))) ||
        (input.activeWar.startTime instanceof Date &&
          Number.isFinite(input.activeWar.startTime.getTime()))),
  );
  const activeWar = hasSafeActiveWarIdentity ? input.activeWar : null;
  const effectiveWarState: "preparation" | "inWar" | "notInWar" =
    currentWarState === "notInWar"
      ? (input.activeWar?.state ?? "notInWar")
      : currentWarState === "inWar"
        ? "inWar"
        : "preparation";
  const resolution = resolveMatchTypeWithPreparedStoredSync({
    opponentTag: activeWar?.opponentTag ?? input.currentWar?.opponentTag ?? "",
    warState: effectiveWarState,
    currentWarId: input.currentWar?.warId ?? null,
    currentWarStartTime: input.currentWar?.startTime ?? null,
    currentWarOpponentTag: input.currentWar?.opponentTag ?? null,
    activeWarId: activeWar?.warId ?? null,
    activeWarStartTime: activeWar?.startTime ?? null,
    activeOpponentTag: activeWar?.opponentTag ?? null,
    existingMatchType: currentMatchType === "UNKNOWN" ? null : currentMatchType,
    existingInferredMatchType: input.currentWar?.inferredMatchType ?? null,
    storedSyncRow: input.storedSyncRow ?? null,
  });
  const effectiveResolution =
    resolution.confirmedCurrent ??
    resolution.storedSync ??
    resolution.unconfirmedCurrent;
  const matchType =
    effectiveResolution?.matchType ??
    (input.currentWar?.inferredMatchType === true && !hasSafeActiveWarIdentity
      ? "UNKNOWN"
      : currentMatchType);
  const currentOutcomeIsWinLose =
    String(input.currentWar?.outcome ?? "").trim().toUpperCase() === "WIN" ||
    String(input.currentWar?.outcome ?? "").trim().toUpperCase() === "LOSE";
  const currentOutcomeConfirmed =
    effectiveResolution?.confirmed === true && currentOutcomeIsWinLose;
  ...
  const projectionActiveWar =
    activeWar &&
    input.projectionWar &&
    input.projectionIdentityWar &&
    compareActiveWarIdentities({
      persisted: {
        warId: input.projectionIdentityWar.warId ?? null,
        warStartTime: input.projectionIdentityWar.startTime ?? null,
        opponentTag: input.projectionIdentityWar.opponentTag ?? null,
      },
      active: {
        warId: input.projectionWar.warId ?? null,
        warStartTime: input.projectionWar.startTime ?? null,
  ...
  const activeSyncNumber =
    matchType === "FWA" && !currentOutcomeConfirmed && projectionActiveWar
  ...
  const safeProjection =
    matchType === "FWA" &&
    !currentOutcomeConfirmed &&
    projectionActiveWar?.startTime instanceof Date &&
    Number.isFinite(projectionActiveWar.startTime.getTime()) &&
    normalizeChecklistClanTag(projectionActiveWar.opponentTag ?? "") &&
    activeSyncNumber !== null &&
    Number.isFinite(Number(activeSyncNumber))
      ? await resolveSafeFwaPointsProjection({
          resolver: input.pointsEstimateResolver ?? pointsEstimateResolver,
          guildId: input.guildId ?? "",
          clanTag: input.clanTag ?? "",
          opponentTag: projectionActiveWar.opponentTag ?? "",
          activeWar: {
            trackedClanTag: input.clanTag ?? "",
            warId: projectionActiveWar.warId ?? null,
            warStartTime: projectionActiveWar.startTime,
            prepStartTime: null,
            opponentTag: projectionActiveWar.opponentTag ?? "",
            syncNumber: activeSyncNumber,
            matchType: "FWA",
            inferredMatchType: input.currentWar?.inferredMatchType ?? null,
            warState: projectionActiveWar.state ?? "preparation",
          },
        }).catch(() => null)
      : null;
  const safeProjectedOutcome = safeProjection
    ? deriveFwaProjectedOutcomeFromPreparedSync({
        clanTag: input.clanTag ?? "",
        opponentTag: projectionActiveWar?.opponentTag ?? activeWar?.opponentTag ?? "",
        clanPoints: safeProjection.clanBalance,
        opponentPoints: safeProjection.opponentBalance,
        syncNum: safeProjection.syncNumber,
      })
    : null;
  const outcomeInferred =
    !currentOutcomeConfirmed && safeProjectedOutcome !== null;
  return {
    matchType,
    outcome: resolveFwaOutcomeFromPreparedEvidence({
      ...
      currentOutcomeConfirmed,
      projectedOutcome: safeProjectedOutcome ?? currentWarProjection.outcome,
      projectedOutcomeIsSafeCurrentSync: safeProjectedOutcome !== null,
      clanTag: input.clanTag ?? null,
      opponentTag: activeWar?.opponentTag ?? input.currentWar?.opponentTag ?? null,
      storedSyncRow: input.storedSyncRow ?? null,
    }),
    inferred:
      effectiveResolution?.inferred === true &&
      (input.currentWar?.inferredMatchType === true ||
...
function resolveChecklistPersistedSyncRow(input: {
  rows: ChecklistPersistedSyncRow[];
  clanTag: string;
  warId?: number | string | null;
  warStartTime?: Date | null;
  opponentTag?: string | null;
}): ChecklistPersistedSyncRow | null {
  const expectedClanTag = normalizeChecklistClanTag(input.clanTag);
  const expectedOpponentTag = normalizeChecklistClanTag(input.opponentTag ?? "");
  const expectedWarId =
    input.warId !== null && input.warId !== undefined && String(input.warId).trim()
      ? String(input.warId)
      : null;
  return (
    input.rows.find((row) => {
      if (row.needsValidation || normalizeChecklistClanTag(row.clanTag) !== expectedClanTag) {
        return false;
      }
      if (expectedOpponentTag && normalizeChecklistClanTag(row.opponentTag) !== expectedOpponentTag) {
        return false;
      }
      if (input.warStartTime instanceof Date) {
        if (!(row.warStartTime instanceof Date) || row.warStartTime.getTime() !== input.warStartTime.getTime()) {
          return false;
        }
      } else if (expectedWarId === null || String(row.warId ?? "") !== expectedWarId) {
        return false;
      }
      if (expectedWarId !== null && row.warId !== null && String(row.warId) !== expectedWarId) {
        return false;
      }
      return true;
    }) ?? null
...
async function loadChecklistPersistedSyncRows(input: {
  guildId: string;
  currentWars: Array<{
    clanTag: string;
    state?: string | null;
...
}): Promise<ChecklistPersistedSyncRow[]> {
  const activeWars = input.currentWars.filter((war) => {
    const state = normalizeWarState(war.state ?? null);
    return (
      (state === "preparation" || state === "inWar" || state === "notInWar") &&
      Boolean(normalizeChecklistClanTag(war.opponentTag ?? "")) &&
      ((war.startTime instanceof Date && Number.isFinite(war.startTime.getTime())) ||
        (war.warId !== null && war.warId !== undefined && Number.isFinite(Number(war.warId))))
    );
  });
  ...
  const starts = [
    ...new Set(
      activeWars
        .map((war) => war.startTime instanceof Date ? war.startTime.toISOString() : null)
  ...
  const warIds = [
    ...new Set(
      activeWars
        .map((war) => war.warId !== null && war.warId !== undefined ? String(war.warId) : null)
        .filter((value): value is string => Boolean(value)),
    ),
  ];
  const identityFilters = [
    starts.length > 0 ? { warStartTime: { in: starts } } : null,
    warIds.length > 0 ? { warId: { in: warIds } } : null,
  ...
  const rows = await prisma.clanPointsSync.findMany({
    where: {
      ...
      clanTag: { in: [...new Set(activeWars.map((war) => `#${normalizeChecklistClanTag(war.clanTag)}`))] },
      OR: identityFilters,
    },
    select: {
      clanTag: true,
...
type MailChecklistWarRenderState = {
  fresh: boolean;
  opponentName: string;
  opponentTag: string | null;
  matchType: "FWA" | "BL" | "MM" | "SKIP" | "UNKNOWN";
  ...
  warId: number | null;
  warStartTime: Date | null;
  timingCandidates: ChecklistExpiryCandidate[];
  liveRevisionFields: {
    warId: number | null;
    opponentTag: string | null;
  } | null;
...
type MailChecklistIdentityResolution =
  | { fresh: true }
  | {
      fresh: false;
      reason:
...
function resolveMailChecklistWarIdentity(input: {
  currentWar: {
    state?: string | null;
    startTime?: Date | null;
    warId?: number | string | null;
  ...
  allowStaleCurrentNotInWar?: boolean;
}): MailChecklistIdentityResolution {
  const currentWar = input.currentWar;
  const liveWar = input.liveWar;
  if (!currentWar || !liveWar) {
    return { fresh: false, reason: "missing_identity" };
  }

  const currentState = normalizeWarState(currentWar.state ?? null);
  if (currentState === "notInWar" && !input.allowStaleCurrentNotInWar) {
    return { fresh: false, reason: "current_not_in_war" };
  }
  if (currentState === "unknown") {
    return {
      fresh: false,
      reason: String(currentWar.state ?? "").trim() ? "current_state_unknown" : "missing_identity",
    };
  }
  if (
    currentState !== "preparation" &&
    currentState !== "inWar" &&
    !(currentState === "notInWar" && input.allowStaleCurrentNotInWar)
  ) {
    return { fresh: false, reason: "current_state_unknown" };
  }

  const liveWarState = normalizeWarState(liveWar.state ?? null);
  if (liveWarState === "notInWar") {
    return { fresh: false, reason: "live_not_in_war" };
  }
  if (liveWarState === "unknown") {
    return {
      fresh: false,
      reason: String(liveWar.state ?? "").trim() ? "live_state_unknown" : "missing_identity",
    };
  }
  if (liveWarState !== "preparation" && liveWarState !== "inWar") {
    return { fresh: false, reason: "live_state_unknown" };
  }
  ...
  const currentWarStartMs =
    currentWar.startTime instanceof Date && Number.isFinite(currentWar.startTime.getTime())
      ...
      : null;
  const currentWarId = currentWar.warId === null || currentWar.warId === undefined
    ? null
    : String(currentWar.warId).trim();
  const liveWarId = liveWar.warId === null || liveWar.warId === undefined
    ? null
    : String(liveWar.warId).trim();
  if (liveWarStartMs !== null && currentWarStartMs !== null && currentWarStartMs !== liveWarStartMs) {
    return { fresh: false, reason: "start_mismatch" };
  }
  if (currentWarId && liveWarId && currentWarId !== liveWarId) {
    return { fresh: false, reason: "start_mismatch" };
  }
  if (
    !(
      liveWarStartMs !== null && currentWarStartMs !== null && currentWarStartMs === liveWarStartMs
    ) &&
    !(currentWarId && liveWarId && currentWarId === liveWarId)
  ) {
    return { fresh: false, reason: "missing_identity" };
  }
  ...
  if (!currentOpponentTag || !liveOpponentTag) {
    return { fresh: false, reason: "missing_identity" };
  }
  if (currentOpponentTag !== liveOpponentTag) {
    return { fresh: false, reason: "opponent_mismatch" };
  }

  return { fresh: true };
}

function resolveMailChecklistWarRenderState(input: {
  currentWar: {
    state?: string | null;
    prepStartTime?: Date | null;
    startTime?: Date | null;
  ...
  clanTag: string;
  allowStaleCurrentNotInWar?: boolean;
}): MailChecklistWarRenderState {
  const identity = resolveMailChecklistWarIdentity({
    currentWar: input.currentWar,
    liveWar: input.liveWar,
    allowStaleCurrentNotInWar: input.allowStaleCurrentNotInWar,
  });
  if (!identity.fresh) {
    console.debug(
      `[fwa_checklist_mail_identity_suppressed] guildId=${input.guildId} clanTag=${input.clanTag} reason=${identity.reason} currentState=${input.currentWar?.state ?? "missing"} liveState=${input.liveWar?.state ?? "missing"}`,
    );
    return {
      fresh: false,
      opponentName: "-",
  ...
  const liveWarOpponentTag = normalizeChecklistClanTag(String(input.liveWar?.opponent?.tag ?? ""));
  const opponentName = sanitizeClanName(String(input.liveWar?.opponent?.name ?? "")) ?? "-";
  const warId =
    normalizeChecklistWarId(input.currentWar?.warId) ??
    normalizeChecklistWarId(input.liveWar?.warId);
  const warStartTime =
    input.currentWar?.startTime ??
    (parseCocApiTime(input.liveWar?.startTime ?? null) !== null
      ? new Date(parseCocApiTime(input.liveWar?.startTime ?? null) as number)
      : null);
  const matchType = normalizeMatchType(input.currentWar?.matchType ?? null);
  const outcome = normalizeOutcome(input.currentWar?.outcome ?? null);
  return {
    fresh: true,
    opponentName,
    opponentTag: liveWarOpponentTag || null,
    matchType,
    outcome: matchType === "FWA" ? outcome ?? "UNKNOWN" : outcome,
    warId,
    warStartTime,
    timingCandidates: buildChecklistWarTimingCandidates({
      prepStartTime: input.currentWar?.prepStartTime ?? null,
      startTime: input.currentWar?.startTime ?? null,
      endTime: input.currentWar?.endTime ?? null,
    }),
    liveRevisionFields: {
      warId,
      opponentTag: liveWarOpponentTag || null,
...
function logMailBaselineDecision(input: {
  guildId: string;
  clanTag: string;
  syncIdentity: string | null;
  reason:
    | "lookup_unavailable"
    | "previous_identity_matched"
    | "previous_identity_mismatch"
    | "previous_row_missing"
    | "verified_war_transition";
  decision?: string;
}): void {
  console.debug(
    `[fwa_checklist_mail_baseline] guild=${input.guildId} clan=${input.clanTag} view=mail sync_identity=${input.syncIdentity ?? "missing"} reason=${input.reason}${input.decision ? ` decision=${input.decision}` : ""}`,
  );
}
...
function getCurrentWarCached(
  ...
  clanTag: string,
  cache?: Map<string, Promise<CurrentWarLookupResult> | CurrentWarLookupResult | any>,
): Promise<CurrentWarLookupResult> {
  const normalizedTag = normalizeChecklistClanTag(clanTag);
  if (typeof cocService?.getCurrentWar !== "function") {
    return Promise.resolve({ status: "unavailable", war: null });
  }
  ...
  const pending = Promise.resolve()
    ...
    .catch((err) => {
      ...
      return { status: "unavailable" as const, war: null };
    });
...
async function buildFwaMatchBasesRenderStateForGuild(params: {
  ...
  for (const clan of trackedClans) {
    ...
    const liveWarLookup = await getCurrentWarCached(
      params.cocService,
      clanTag,
      params.warLookupCache,
    );
    const liveWar = liveWarLookup.war;
    const liveWarIdentity = resolveChecklistLiveWarIdentity(liveWar);
    const liveWarIsActive =
      liveWarIdentity?.state === "preparation" || liveWarIdentity?.state === "inWar";
    const reconciliationLiveWar = liveWar
      ? { ...liveWar, clan: { name: liveWar.clan?.name ?? clan.name } }
      : liveWar;
    const reconciliation =
      params.guildId && liveWarIsActive
  ...
  for (const clan of trackedClans) {
    ...
    if (effectiveCurrentWarState === "notInWar") {
      console.debug(
        `[fwa_checklist_bases_row] guildId=${params.guildId} clanTag=${clanTag} visibleReaction=false matchSource=skipped syncIdentitySource=${currentSyncIdentitySource} syncMessageId=${currentSyncIdentity ?? "missing"} baseSwapReason=skipped baseSwapSyncMessageId=missing completionReason=skipped finalStatus=skipped currentWarState=${effectiveCurrentWarState}`,
      );
      rows.push({
        clanTag,
    ...
    const currentBaseSwap = activeCurrentWar
      ? await trackedMessageService
          ...
          .catch(() => null)
      : null;
    const persistedSyncRow = resolveChecklistPersistedSyncRow({
      rows: persistedSyncRows,
      clanTag,
      warId: activeCurrentWar?.warId ?? null,
    ...
    const issueSummary = currentBaseSwap
      ? buildFwaBaseSwapIssueSummary(
          ...
          effectiveMatchState.matchType === "UNKNOWN" ? null : effectiveMatchState.matchType,
        )
      : {
          hasIssues: false,
    ...
    const statusText = issueSummary.hasIssues
      ...
      : allGoodCompletion
        ...
        : "\u274c Bases not checked";
    console.debug(
      `[fwa_checklist_bases_row] guildId=${params.guildId} clanTag=${clanTag} visibleReaction=${visibleReaction} matchSource=${effectiveMatchState.inferred ? "inferred" : "confirmed"} matchType=${matchType} outcome=${outcome ?? "none"} persistedOutcome=${persistedSyncRow?.outcome ?? persistedSyncRow?.lastKnownOutcome ?? "none"} syncIdentitySource=${rowSyncIdentitySource} syncMessageId=${rowSyncIdentity ?? "missing"} baseSwapReason=${baseSwapSource} baseSwapSyncMessageId=${currentBaseSwapSyncIdentity ?? "missing"} completionReason=${completionSource} finalStatus=${basesStatus}`,
    );
    rows.push({
      clanTag,
      compactCopyLine: `${clanLabel} | ${matchStateEmoji} | ${statusText}${effectiveMatchState.inferred || effectiveMatchState.outcomeInferred ? " ⚠️" : ""}`,
      badgeEmojiId: clanBadge.badgeEmojiId,
      badgeEmojiName: clanBadge.badgeEmojiName,
      badgeEmojiInline: clanBadge.badgeEmojiInline,
      basesStatus,
      matchType,
      matchStateInferred: effectiveMatchState.inferred,
      outcome,
      warId: activeCurrentWar?.warId ?? null,
      opponentTag: activeCurrentWar?.opponentTag ?? null,
  ...
  return {
    ...
    emptyMessage: null,
    expectedTrackedClanTags: trackedClans.map((clan) => normalizeChecklistClanTag(clan.tag)),
  };
}
...
function buildRowsFromCopyView(params: {
  ...
  return params.orderedTags.flatMap((tag, index) => {
    ...
    const rowMetadata = params.rowMetadataByTag?.get(normalizedTag);
    return [
      {
...
function preserveMailRowsAfterUnavailableLookup(params: {
  guildId: string;
  currentSyncIdentity: string | null;
  previousSyncIdentity?: string | null;
  previousRows?: FwaMatchChecklistTrackedRow[] | null;
  rows: FwaMatchChecklistTrackedRow[];
  currentWarByTag: Map<string, {
    state?: string | null;
    warId?: number | string | null;
    opponentTag?: string | null;
  }>;
  ...
  const previousRowsByTag = new Map<string, FwaMatchChecklistTrackedRow>();
  for (const row of params.previousRows ?? []) {
    const clanTag = normalizeChecklistClanTag(row.clanTag);
    if (clanTag && !previousRowsByTag.has(clanTag)) {
      previousRowsByTag.set(clanTag, row);
    }
  }
  const currentSyncIdentity = normalizeTrackedMessageId(params.currentSyncIdentity);
  const previousSyncIdentity = normalizeTrackedMessageId(params.previousSyncIdentity ?? null);

  return params.rows.map((row) => {
    const clanTag = normalizeChecklistClanTag(row.clanTag);
    const lookup = params.liveWarLookupByTag.get(clanTag);
    if (lookup?.status !== "unavailable") {
      if (lookup?.war && normalizeWarState(lookup.war.state ?? null) === "notInWar") {
        logMailBaselineDecision({
          guildId: params.guildId,
          clanTag,
          syncIdentity: currentSyncIdentity,
          reason: "verified_war_transition",
        });
      }
      return row;
    }

    const previousRow = previousRowsByTag.get(clanTag);
    if (!previousRow) {
      logMailBaselineDecision({
        guildId: params.guildId,
        clanTag,
        ...
        decision: "previous_row_missing",
      });
      return row;
    }

    const currentWar = params.currentWarByTag.get(clanTag) ?? null;
    const currentState = normalizeWarState(currentWar?.state ?? null);
    const currentContextKey = currentWar
      ? buildFwaMatchChecklistRowContextKey({
          clanTag,
      ...
      : null;
    const previousContextKey = String(previousRow.contextKey ?? "").trim() || null;
    const priorWarId = String(previousRow.warId ?? "").trim();
    ...
    const identityMatched =
      ...
      currentContextKey === previousContextKey &&
    ...
    if (identityMatched) {
      logMailBaselineDecision({
        ...
        syncIdentity: currentSyncIdentity,
        reason: "lookup_unavailable",
        decision: "previous_identity_matched",
      });
      logMailBaselineDecision({
        guildId: params.guildId,
        clanTag,
        syncIdentity: currentSyncIdentity,
        reason: "previous_identity_matched",
      });
      return { ...previousRow };
    }
...
export async function buildFwaMatchChecklistRenderStateForGuild(params: {
  ...
  if ((params.viewType ?? "Mail") === "Bases") {
    return buildFwaMatchBasesRenderStateForGuild({
      ...
      warLookupCache: params.warLookupCache,
  ...
  const liveWarByTag = new Map<string, any | null>();
  const liveWarLookupByTag = new Map<string, CurrentWarLookupResult>();
  const reconciliationByTag = new Map<string, Awaited<ReturnType<typeof reconcileActiveWarIdentity>> | null>();
  for (const clan of trackedClans) {
    const clanTag = normalizeChecklistClanTag(clan.tag);
    const currentWar = currentWarByTag.get(clanTag) ?? null;
    ...
    const reconciliation =
      params.guildId && liveWarIsActive
        ...
        : null;
    liveWarByTag.set(clanTag, liveWar);
    liveWarLookupByTag.set(clanTag, liveWarLookup);
    reconciliationByTag.set(clanTag, reconciliation);
    if (reconciliation?.currentWar) {
      currentWarByTag.set(clanTag, reconciliation.currentWar as Exclude<typeof currentWar, null>);
    }
  ...
  const outcomeByTag = new Map<string, "WIN" | "LOSE" | "UNKNOWN" | null>();
  const rowMetadataByTag = new Map<string, {
    warId: number | string | null;
    opponentTag: string | null;
    warStartTimeIso: string | null;
    matchType: "FWA" | "BL" | "MM" | "SKIP" | "UNKNOWN";
  ...
  for (const clan of trackedClans) {
    ...
    const mailRenderState = resolveMailChecklistWarRenderState({
      currentWar: effectiveCurrentWar
        ? {
            ...
            endTime: effectiveCurrentWar.endTime ?? null,
    ...
    const persistedSyncRow = mailRenderState.fresh
      ? resolveChecklistPersistedSyncRow({
          rows: persistedSyncRows,
          clanTag,
    ...
    const mailStatus = mailRenderState.fresh
      ? await warMailLifecycleService.resolveStatusForCurrentWar({
          client: params.client,
          guildId: params.guildId,
          clanTag,
          warId: mailRenderState.warId,
          warStartTime: mailRenderState.warStartTime,
          opponentTag: mailRenderState.opponentTag,
    ...
    const compactCopyLine = buildFwaMatchCompactCopyLine({
      ...
      clanName,
      opponentName: mailRenderState.opponentName,
      opponentTag: mailRenderState.opponentTag,
    ...
    rowMetadataByTag.set(clanTag, {
      warId: mailRenderState.warId,
      opponentTag: mailRenderState.opponentTag,
      warStartTimeIso:
        mailRenderState.warStartTime instanceof Date &&
        Number.isFinite(mailRenderState.warStartTime.getTime())
          ? mailRenderState.warStartTime.toISOString()
          : null,
      matchType: effectiveMatchState.matchType,
    });
    inferredByTag.set(clanTag, effectiveMatchState.inferred);
    outcomeByTag.set(clanTag, effectiveMatchState.outcome);
    copyLines.push(
      `${compactCopyLine}${effectiveMatchState.inferred || effectiveMatchState.outcomeInferred ? " ⚠️" : ""}`,
    );
  }
  ...
  const rows = buildRowsFromCopyView({
    ...
    ...row,
    matchStateInferred: inferredByTag.get(normalizeChecklistClanTag(row.clanTag)) === true,
    outcome: outcomeByTag.get(normalizeChecklistClanTag(row.clanTag)) ?? null,
  }));
  const preservedRows = preserveMailRowsAfterUnavailableLookup({
    guildId: params.guildId,
    currentSyncIdentity,
    previousSyncIdentity: params.previousSyncIdentity ?? null,
    previousRows: params.previousRows ?? null,
    rows,
    currentWarByTag,
    liveWarLookupByTag,
  });
