...
export function formatParticipationBackfillPlan(plan: ParticipationBackfillPlan, apply: boolean): string {
  ...
  lines.push(
    `${MEMBERSHIP_PARTICIPATION_BACKFILL_PREFIX} summary ` + [
      ...
      `validated_points_considered=${summary.validatedPointsConsidered}`,
      `dirty_points_ignored=${summary.dirtyPointsIgnored}`,
      `points_canonicalized=${summary.pointsCanonicalized}`,
      `stale_raw_war_ids_canonicalized=${summary.staleRawWarIdsCanonicalized}`,
      `unmatched_validated_points=${summary.unmatchedValidatedPoints}`,
      `ambiguous_tuple_points=${summary.ambiguousTuplePoints}`,
