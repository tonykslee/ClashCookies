...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  cwl: {
    ...
    details: [
      ...
      "`/cwl rotations import` is admin-only by default and imports active planner tabs from one public Google Sheet after a confirmation preview and row-review step. Structural rows may be skipped automatically, but player-like rows are never dropped silently. Public imports do not require Google Sheets credentials; export/write still does.",
