...
const ADMIN_DEFAULT_TARGETS = new Set<string>([
  ...
  "autorole",
  "autorole:config",
  "autorole:delayed-signup-role",
  "autorole:delayed-signup-role:add",
  "autorole:delayed-signup-role:remove",
  "autorole:delayed-signup-role:list",
  "autorole:delayed-signup-role:clear",
  "autorole:rules",
  "autorole:exclusions",
  "defer:config",
  "fillers",
...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "clan": {
    ...
    details: [
      ...
      "`list` without `type` shows one embed grouped by FWA/CWL/RAIDS; `type:FWA|CWL|RAIDS` switches to the per-registry view and accepts `display:minimal|detailed` (default `minimal`). Overview/minimal rows show current member counts when persisted, no longer show `leadRole`, and include a member-count `Refresh` button; CWL minimal rows show the CWL league emoji plus a short league abbreviation when ranked, keep `UNK` for unranked/unknown, and show spin status as emoji only. `display:detailed` opens the drilldown, detailed FWA still shows `leadRole` and configured reps, detailed CWL shows the clan link/tag, CWL league emoji, spin status emoji, observed CWL roster count, current clan member count, and active roster link/title with a manual refresh button, and detailed RAIDS keeps its refresh button. Detailed CWL rows sort by CWL league descending, then active roster title ascending. If `type` is omitted, `display` is ignored.",
  ...
  autorole: {
    ...
    details: [
      ...
      "`delayed-signup-role add/remove/list/clear` manage the persisted delayed-signup role IDs used later by roster/signup policy. `add` and `remove` use a Discord role picker, `list` shows the configured mentions and count, and `clear` removes every delayed-signup role.",
      "`config show` displays the current guild autorole policy snapshot, including the delayed-signup role list, nickname sync status, and template.",
      ...
      "`config`, `delayed-signup-role`, `rules`, and `exclusions` stay admin-only by default; `refresh` is FWA Leader role + Administrator by default.",
    ],
    examples: [
      "/autorole refresh",
      ...
      "/autorole delayed-signup-role add role:@Visitor",
      "/autorole delayed-signup-role remove role:@Visitor",
      "/autorole delayed-signup-role list",
      "/autorole delayed-signup-role clear",
      "/autorole rules list",
  ...
  roster: {
    ...
    details: [
      ...
      "`/roster show` displays the guild default roster board columns, the current source (`Server override` or `Built-in`), the built-in fallback layout, and the exact available column keys from the registry.",
      "`/roster set columns:<keys>` saves the guild default roster board columns using comma-separated exact keys such as `townhall_icons,discord_username,player_name,player_tag`. `/roster reset` removes the guild override and restores the built-in layout without changing any per-roster customization.",
  ...
  link: {
    ...
    details: [
      ...
      "`list` renders non-zero linked/unlinked count buckets with compact rows and a Columns dropdown that lets you choose 1-5 visible fields. Available columns are Town Hall, Player Name, Discord Display, Discord Username, Weight, Inactivity, Clan Role, and Player Tag. Linked rows always start with a Unicode `?` status marker, show Town Hall as a bare number or `?` label inside inline code, and keep the player name visible; unlinked rows use the same layout with a Unicode `?` status marker. `Player Tag` stays in its own inline-code cell for mobile tap/copy, while filler rows can show a trailing `??` marker outside inline code. `Player Tags` is the only mode that shows the player tag. Sort controls row order while columns control visible fields; selected columns are preserved across Refresh Data, sort cycling, and tracked-clan switching. `Clan Role` shows `lead`, `co`, `eld`, or `mem`, sorted by leader, co-leader, elder/admin, then member.",
  ...
  fwa: {
    ...
    details: [
      ...
      "Confirm-and-send pings the tracked clan role (`TrackedClan.clanRoleId`) when pinging is enabled, and CWL swap-back reminders can use `ping_role` instead of the tracked clan role.",
  ...
  "bot-logs": {
    ...
    details: [
      ...
      "Use `/bot-logs` with no arguments to list all bot-log configurations in the guild, or `/bot-logs type:base-swap` / `type:ban-log` / `type:ban-join-alert` / `type:maintenance|sync|checklist` to inspect one specific configuration.",
  ...
  permission: {
    ...
    details: [
      ...
      "`/permission list` includes `fwa:compliance`, `fwa:weight-*`, and `defer*` targets; most defer targets default to FWA leader role + Administrator, while `defer:config` stays Administrator-only by default.",
