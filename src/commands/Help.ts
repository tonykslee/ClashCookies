...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  fwa: {
    ...
    details: [
      ...
      "Default is `type:Mail`. `type:Mail` public posts are the persistent reaction-driven mail checklist with auto-pinning and `Refresh`; `type:Mail` private is snapshot-only. `type:Bases` public posts are the reaction-driven bases checklist, and `type:Bases` private is snapshot-only.",
