  ...
  link: {
    ...
    details: [
      ...
      "`status` shows the persisted trust state for one or more of your linked player tags, including source, verification status, verification method, and verified timestamps.",
        "`list` renders non-zero linked/unlinked count buckets with compact rows: linked rows start with a resolved `✅` status emoji, show the Town Hall as compact `TH<n>` text inside inline code, show the player tag only in `Player Tags` sort mode, and keep the player/name/weight text in padded inline code; unlinked rows use the same layout with a resolved `❌` status emoji, while filler rows can show a trailing `🧍` marker outside inline code.",
        "Weight (`Wt`) comes from `FwaClanMemberCurrent.weight` first, then `FwaPlayerCatalog.latestKnownWeight`, then `PlayerCurrent.currentWeight`, and is shown as compact lowercase `k` text (for example `145k`), with `—` when no resolved positive weight exists.",
