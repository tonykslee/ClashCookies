...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  raids: {
    ...
    details: [
      ...
      "`/raids overview` focuses on raid status and sort order; the overview includes a tracked-clan dropdown, refresh button, status emojis (`⚔️` for ongoing/incomplete raids, `🌄` for completed raids), clickable clan titles, an intel line when current-season layout marks exist, and open defending-clan rows when the live defense log can resolve them. Rows sort by ongoing raid, raids completed, current-season default layout count, and then the tracked display order. `clan` opens a single-clan view with attack and defense detail sections when the live raid-season source can resolve them, and both views keep the same refresh button so the message updates in place. `/raids overview clan:<tag>` keeps the detailed attack/completed stats, the drilldown adds a compact grade-grouped intel summary above the attack/defense sections when marks exist, and the defending section shows attacks used plus districts remaining for each attacking clan with `🔓` for open clans and `🔒` for closed, invite-only, or unknown clans. `/raids intel` opens a raid-intel view for one tracked clan with defender/district details, the tracked-clan line renders `| 🏘️ {upgrades}` inline, optional district pre-mark args such as `capital_peak:Custom - Hard`, and the same interactive district controls after the embed opens. Supplying `upgrades:<number>` persists that value back to the tracked RAID clan row so later intel refreshes show it again.",
    ],
    examples: [
      "/raids overview",
  ...
  compo: {
    ...
    details: [
      ...
      "`place`: suggest placement by war weight from persisted ACTUAL current-member state (`TrackedClan` + `FwaClanMemberCurrent` + `HeatMapRef`) using the same ACTUAL Auto-Detect projection path as `/compo state` and `/compo advice`, with projected totals selecting the target band and resolved-roster deltas driving recommendations.",
  ...
  fwa: {
    ...
    details: [
      ...
      "Use `/fwa match copy_paste:true checklist:true visibility:public` to post the reaction-driven checklist version; checklist reactions require `visibility:public`.",
    ...
    examples: [
      ...
      "/fwa match copy_paste:true checklist:true visibility:public",
