...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "clan": {
    ...
    details: [
      ...
      "`list` without `type` shows one embed grouped by FWA/CWL/RAIDS; `type:FWA|CWL|RAIDS` switches to the per-registry view and accepts `display:minimal|detailed` (default `minimal`). Overview/minimal rows show current member counts when persisted, no longer show `leadRole`, and include a member-count `Refresh` button; `display:detailed` opens the drilldown, detailed FWA still shows `leadRole`, detailed CWL shows the clan link/tag, CWL league, spin status, observed CWL roster count, current clan member count, and active roster link/title with a manual refresh button, and detailed RAIDS keeps its refresh button. Detailed CWL rows sort by CWL league descending, then active roster title ascending. If `type` is omitted, `display` is ignored.",
