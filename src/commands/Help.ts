...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "clan": {
    ...
    details: [
      ...
      "`list` without `type` shows one embed grouped by FWA/CWL/RAIDS; `type:FWA|CWL|RAIDS` switches to the per-registry view. FWA list rows show `leadRole` when configured. RAIDS list rows use the stored clan name, show join status with 🔒/🔓, and include an inline refresh button to re-sync missing names or join status.",
  ...
  roster: {
    ...
    details: [
      ...
      "`/roster manage roster:<roster> action:<add|move|remove|change_roster|set_weight|open|close|archive> [user:<discordUser>] ...` is the roster-keyed manager surface for mutation, manual fallback weight entry, and lifecycle changes. For add/move/remove/change_roster/set_weight, selecting a user opens the interactive account picker; `move` uses `target_group` as the destination group and defaults to `Confirmed` when omitted; resolved roster weights prefer `FwaPlayerCatalog.latestKnownWeight` first, then `ExternalPlayerWeightCurrent` as the fallback owner when needed.",
  ...
  "bot-logs": {
    ...
    details: [
      ...
      "Use `/bot-logs` with no arguments to view the currently configured generic channel mention, `/bot-logs type:base-swap` to view the typed base-swap channel, or `/bot-logs type:maintenance` to view the typed maintenance channel.",
