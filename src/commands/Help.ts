...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "clan-health": {
    ...
    details: [
      "Shows a persisted war-plan compliance summary over the tracked metrics window (omit window for the latest 30 syncs; configure 7 to 180 days when supplied) with violations, distinct violating player accounts, currently linked Discord users involved, and affected/evaluated FWA wars.",
      ...
      "Tracked Clan Health results provide a first ephemeral action row with View Inactive, View Unlinked, View Compo, View Violations, and War History, plus a second row with View Trends; each opens a clan-scoped workflow with its permission target. Omitted window uses the latest 30 syncs; an explicit 7-to-180 value uses days. War History shows persisted ended wars from that selected window and renders the latest 10. Trends shows captured sync-boundary readiness history from that selected window. External clan views do not include these buttons.",
  ...
  fwa: {
    ...
    details: [
      ...
      "`/fwa weight-age` reports last submitted weight age from persisted `FwaClanCatalog.weightSubmitDate` for one clan or all tracked clans; missing catalog data is shown as unknown.",
      "`/fwa weight-link` returns fwastats weight page URL(s) for one clan or all tracked clans.",
      "`/fwa weight-health` summarizes persisted catalog dates for all tracked clans and flags stale weight submissions (outdated >7d, severe >=30d); it performs no live FWAStats request.",
      ...
      "Default access for `/fwa weight-age`, `/fwa weight-link`, and `/fwa weight-health` is FWA leader role + Administrator (or override via `/permission add`).",
  ...
  "bot-logs": {
    ...
    details: [
      ...
      "Use `/bot-logs` with no arguments to list all bot-log configurations in the guild, or `/bot-logs type:base-swap` / `type:ban-log` / `type:ban-join-alert` / `type:clan-goals` / `type:sync-retrospective` / `type:maintenance|sync|checklist` to inspect one specific configuration.",
