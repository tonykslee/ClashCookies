...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "clan": {
    ...
    details: [
      ...
      "`rep timezone` stores a rep profile timezone in the dedicated rep profile owner, is administrator-only by default, validates the player is an assigned rep, and previews the current local time after saving.",
      "`rep time [clan] [visibility:private|public]` is read-only, is available to the configured FWA leader role and administrators, shows every configured rep grouped by assigned tracked clan in deterministic tracked-clan order, and renders the current local time for each rep without calling live CoC APIs.",
    ...
    examples: [
      ...
      "/clan rep timezone player:#PYLQ0289 timezone:PST",
      "/clan rep time",
