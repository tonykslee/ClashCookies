...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  link: {
    ...
    details: [
      ...
      "`status` shows the persisted trust state for one or more of your linked player tags, including source, verification status, verification method, and verified timestamps.",
        "`list` renders non-zero linked/unlinked count buckets with padded rows: linked rows start with a resolved `yes` status emoji, place the Town Hall icon outside inline code, show the player tag only in `Player Tags` sort mode, and keep the player/name/weight text in padded inline code; unlinked rows use the same layout with a resolved `no` status emoji, while filler rows can show a trailing `:person_standing:` marker outside inline code.",
  ...
  recruitment: {
    ...
    details: [
      ...
      "`countdown settings reminders:true|false` mutes or unmutes recruitment cooldown reminder pings for your countdown timers without deleting them; muted timers still appear in status/dashboard.",
