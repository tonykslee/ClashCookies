...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "bot-logs": {
    ...
    details: [
      ...
      "Use `/bot-logs type:clan-goals [enable:<clan-log channel|clan-lead channel|bot-log channel|custom|false>] [channel:<channel-or-thread>]` to configure live war and canonical war-end clan-goal notifications. It defaults to disabled, and `channel` is only used with `enable:custom`; the sync goal is not wired yet.",
      ...
      "Use `/bot-logs` with no arguments to list all bot-log configurations in the guild, or `/bot-logs type:base-swap` / `type:ban-log` / `type:ban-join-alert` / `type:clan-goals` / `type:maintenance|sync|checklist` to inspect one specific configuration.",
