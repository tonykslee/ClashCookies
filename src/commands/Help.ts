...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  roster: {
    ...
    details: [
      ...
      "`/roster manage roster:<roster> action:<add|move|remove|set_weight|open|close|archive> [user:<discordUser>] ...` is the roster-keyed manager surface for mutation, manual fallback weight entry, and lifecycle changes. For add/move/remove/set_weight, selecting a user opens the interactive account picker; resolved roster weights prefer `FwaPlayerCatalog.latestKnownWeight` first, then `ExternalPlayerWeightCurrent` as the fallback owner when needed.",
