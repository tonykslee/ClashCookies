...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  fwa: {
    ...
    details: [
      ...
      "Use `/fwa match-checklist type:Mail visibility:public` to post the persistent reaction-driven mail checklist version with reactions; `visibility:private` renders an ephemeral snapshot without reactions. `type:Bases` posts a tracked bases checklist in public mode with clan badge reactions and auto-pinning, while `visibility:private` renders an ephemeral snapshot without reactions. It supports `clan:<tag-or-short-name>` plus `checked:true|false` to save or clear durable current-war completion state.",
