...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  layout: {
    ...
    details: [
      "`/layout link:<link> [title] [description] [image] [img-url]` creates or reuses a generic tracked LayoutRecord; Town Hall is derived automatically from the shared Clash parser.",
  ...
  fwa: {
    ...
    details: [
      ...
      "`/fwa layout [th:<8-18>] [type:RISINGDAWN|BASIC|ICE] [link:<Clash layout link>] [title:<text>] [description:<text>] [img-url:<http(s) URL>]` lists canonical FWA layouts, looks one up, or lets administrators assign a canonical link. Update TH is derived from the link; the optional `th` must match when supplied. Replies are ephemeral, use canonical Discord post jump links, and do not echo raw Clash URLs. Reads are accessible by default; only update mode is administrator-gated.",
