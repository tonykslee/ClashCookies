...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  cwl: {
    ...
    details: [
      ...
      "`/cwl rotations create` is admin-only by default and only works during persisted CWL preparation state for the tracked clan. Optional `roster:<id>` seeds the plan from the confirmed group on one open/closed CWL roster for that clan, using persisted weight data when available.",
  ...
  roster: {
    ...
    examples: [
      "/roster create category:CWL clan:#2QG2C08UP name:CWL Alpha Signup timezone:America/Los_Angeles import_members:true",
      "/roster list",
      "/roster post roster:roster_123",
      "/roster manage roster:roster_123 action:add group:confirmed players:#PQL0289,#QGRJ2222",
      "/roster manage roster:roster_123 action:move group:substitute players:#PQL0289",
      "/roster manage roster:roster_123 action:remove players:#PQL0289",
      "/roster manage roster:roster_123 action:open",
      "/roster edit roster:roster_123 name:CWL Alpha Signup timezone:America/New_York",
