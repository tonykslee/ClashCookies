  ...
  link: {
    ...
    details: [
      ...
      "`status` shows the persisted trust state for one or more of your linked player tags, including source, verification status, verification method, and verified timestamps.",
        "`list` renders non-zero linked/unlinked count buckets with padded rows: linked rows start with a resolved `yes` status emoji, place the Town Hall icon outside inline code, show the player tag in its own inline-code segment, and keep the player/name/weight text in padded inline code; unlinked rows use the same layout with a resolved `no` status emoji and player tag shown in its own inline-code segment. Filler rows can show a trailing `:person_standing:` marker outside inline code.",
