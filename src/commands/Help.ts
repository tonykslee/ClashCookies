...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  raids: {
    ...
    details: [
      ...
      "`/raids overview` focuses on raid status and sort order; the overview includes a tracked-clan dropdown, refresh button, status emojis (`⚔️` for ongoing/incomplete raids, `🌄` for completed raids), clickable clan titles, and open defending-clan rows when the live defense log can resolve them. `clan` opens a single-clan view with attack and defense detail sections when the live raid-season source can resolve them, and both views keep the same refresh button so the message updates in place. `/raids overview clan:<tag>` keeps the detailed attack/completed stats. `/raids intel` opens a raid-intel view for one tracked clan with defender/district details, saved layout grades, and interactive district controls.",
    ],
