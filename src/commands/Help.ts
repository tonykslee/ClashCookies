...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  compo: {
    ...
    details: [
      ...
      "`heatmapref`: renders the persisted HeatMapRef table as an attached PNG image with `TH11+`, `Match%`, and `Clans` columns, plus an inline `Copy Table` button that returns the exact tab-separated table text without triggering any rebuild/update work.",
