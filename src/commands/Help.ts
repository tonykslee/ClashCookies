...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  fwa: {
    ...
    details: [
      ...
      "Use `/fwa match-checklist type:Mail visibility:public` to post the persistent reaction-driven mail checklist version with reactions; `visibility:private` renders an ephemeral snapshot without reactions. `type:Bases` renders a read-only bases checklist snapshot with no reactions or pinning lifecycle, and supports `clan:<tag-or-short-name>` plus `checked:true|false` to save or clear durable current-war completion state.",
