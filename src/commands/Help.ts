...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "clan-health": {
    ...
    details: [
      ...
      "Tracked Clan Health results provide a first ephemeral action row with View Inactive, View Unlinked, View Compo, View Violations, and War History, plus a second row with View Home Roster, View Away, View Transfers, and View Trends; each opens a clan-scoped workflow with its permission target. Home Roster is a read-only account/playerTag-level view: active Home periods reserve slots while Away, only fresh CURRENT roster coverage derives Present/Away, STALE or UNAVAILABLE coverage remains Unknown, current unassigned occupants stay diagnostic, and pending transfers have no decision controls yet. Omitted window drilldowns use the latest 30 sync numbers; an explicit 7-to-180 window uses days. War History shows persisted ended wars, newest first, with the latest 10 displayed; Trends shows captured sync-boundary readiness history. External clan views do not include these buttons.",
