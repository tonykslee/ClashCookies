...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  link: {
    ...
    details: [
      ...
      "`list` renders non-zero linked/unlinked count buckets with compact rows: linked rows start with a Unicode `✅` status marker, show Town Hall as a bare `18` / `?` label inside inline code, always show the player name, and append one optional mode-specific value block; unlinked rows use the same layout with a Unicode `❌` status marker, while filler rows can show a trailing `🧍` marker outside inline code. `Player Tags` is the only mode that shows the player tag. `Clan Role` shows `lead`, `co`, `eld`, or `mem`, sorted by leader, co-leader, elder/admin, then member.",
