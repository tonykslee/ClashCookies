  ...
  link: {
    ...
    details: [
      ...
      "`status` shows the persisted trust state for one or more of your linked player tags, including source, verification status, verification method, and verified timestamps.",
        ...
        "`list` includes a tracked-clan dropdown and a sort-cycle button (`Discord Name -> Weight Desc -> Player Tags -> Player Name -> Clan Rank Desc -> Inactivity`) and updates the same message in place. `Clan Rank Desc` shows the current in-game clan rank value (for example `#17`) for each row instead of weight. `Inactivity` shows days inactive and missed wars using persisted activity/history data.",
