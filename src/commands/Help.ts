...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "clan": {
    ...
    details: [
      ...
      "`list` without `type` shows one embed grouped by FWA/CWL/RAIDS; `type:FWA|CWL|RAIDS` switches to the per-registry view and accepts `display:minimal|detailed` (default `minimal`). Overview/minimal rows show current member counts when persisted, no longer show `leadRole`, and include a member-count `Refresh` button; CWL minimal rows use the CWL league emoji as the leading marker when ranked, keep `-` for unranked/unknown, and show spin status as emoji only. `display:detailed` opens the drilldown, detailed FWA still shows `leadRole`, detailed CWL shows the clan link/tag, CWL league emoji, spin status emoji, observed CWL roster count, current clan member count, and active roster link/title with a manual refresh button, and detailed RAIDS keeps its refresh button. Detailed CWL rows sort by CWL league descending, then active roster title ascending. If `type` is omitted, `display` is ignored.",
  ...
  fwa: {
    ...
    details: [
      ...
      "For `type:Bases`, `clan:<tag-or-short-name> checked:true|false` saves or clears durable current-war all-good state. Bases reminders go to the clan's configured leader/notify/log channel at 12h, 6h, 3h, and 1h left.",
