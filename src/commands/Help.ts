...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  "bot-logs": {
    ...
    details: [
      ...
      "Use `/bot-logs type:clan-goals [enable:<clan-log channel|clan-lead channel|bot-log channel|custom|false>] [channel:<channel-or-thread>]` to configure live war clan-goal notifications. It defaults to disabled, and `channel` is only used with `enable:custom`; the no-violations, no-missed-attacks, and sync goals are not wired yet.",
