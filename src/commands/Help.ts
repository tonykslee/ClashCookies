...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  roster: {
    ...
    details: [
      ...
      "`/roster edit roster:<roster>` edits roster metadata such as name, category, clan scope, limits, town-hall gates, signup role requirement, roster role, sort order, timezone fields, and `end_time`. `end_time` remains the self-service signup deadline and automatic closure time; authorized manager maintenance remains available after it. `visitor_signup_open_time` is interpreted in the roster timezone, `clear_visitor_signup_open_time:true` removes it without deleting the configured delayed roles, and verified/family members plus roster managers keep their existing bypass behavior. `name` is the supported label for edit.",
  ...
  fwa: {
    ...
    details: [
      ...
      "`/fwa match` auto-resolves current war opponent from CoC API and evaluates win/lose/tiebreak using cached points + the shared local active-cycle read model; stale points-site rows do not replace a safely resolved local sync, while equal-point tie-breaks remain unresolved until parity is known and unequal points still determine WIN/LOSE; `copy_paste:true` opens the compact copy/paste view directly.",
