...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  roster: {
    ...
    details: [
      ...
      "`/roster` settings actions currently include roster info, customize, close roster, clear roster, hide buttons, archive mode, unregistered members, and missing members; destructive actions require confirmation where applicable.",
  ...
  link: {
    ...
    details: [
      ...
      "Weight (`Wt`) comes from `FwaPlayerCatalog.latestKnownWeight` first, then the manual `ExternalPlayerWeightCurrent` fallback when needed, and is shown as compact lowercase `k` text (for example `145k`), with `—` when missing.",
