  ...
  link: {
    ...
    details: [
      ...
      "`status` shows the persisted trust state for one or more of your linked player tags, including source, verification status, verification method, and verified timestamps.",
        "`list` renders non-zero linked/unlinked count buckets with compact rows: linked rows start with a Unicode `✅` status marker, show the Town Hall as compact `TH<n>` text inside inline code, show the player tag only in `Player Tags` sort mode, and keep the player/name/weight text in padded inline code; unlinked rows use the same layout with a Unicode `❌` status marker, while filler rows can show a trailing `🧍` marker outside inline code.",
        ...
        "`list` reads persisted current-member rows by default and includes a tracked-clan dropdown plus `Refresh Data` / sort buttons. `Refresh Data` reloads the selected clan's live CoC membership, persists the result, and rerenders the same message from DB. The sort-cycle button rotates `Discord Name -> Weight Desc -> Player Tags -> Player Name -> Clan Rank Desc -> Inactivity` and updates the same message in place. `Clan Rank Desc` shows the current in-game clan rank value (for example `#17`) for each row instead of weight. `Inactivity` uses the same missed-war data as `/inactive wars`; rows show missed-war count as `— 2w` and days are shown as `—` unless a reliable shared days source is available.",
