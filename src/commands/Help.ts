...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  ...
  warplan: {
    summary: "Manage clan-scoped custom war plans by match type and FWA outcome.",
    details: [
      "`set` saves custom text for one clan (`clan-tag`) for BL/MM, or FWA WIN/LOSE plans.",
      ...
      "For FWA, omitting `outcome` applies the same plan text to WIN and both LOSE lose styles.",
      "`show` supports all plans, one match type, one FWA outcome, or one FWA LOSE lose-style path.",
      "`reset` clears custom entries and falls back to default plan text.",
    ],
    examples: [
      "/warplan set clan-tag:2QG2C08UP match-type:BL plan-text:Everyone switch to war base.",
      "/warplan set clan-tag:2QG2C08UP match-type:FWA outcome:WIN plan-text:Mirror 3 then cleanup 2.",
      "/warplan set clan-tag:2QG2C08UP match-type:FWA outcome:LOSE lose-style:TRIPLE_TOP_30 plan-text:Top 30 only.",
      "/warplan set clan-tag:2QG2C08UP match-type:FWA plan-text:Follow mail instructions.",
      "/warplan show clan-tag:2QG2C08UP",
      "/warplan show clan-tag:2QG2C08UP match-type:FWA",
      "/warplan show clan-tag:2QG2C08UP match-type:FWA outcome:LOSE lose-style:TRADITIONAL",
      "/warplan reset clan-tag:2QG2C08UP",
      "/warplan reset clan-tag:2QG2C08UP match-type:MM",
      "/warplan reset clan-tag:2QG2C08UP match-type:FWA outcome:LOSE lose-style:TRIPLE_TOP_30",
    ],
  },
  accounts: {
  ...
  permission: {
    ...
    details: [
      ...
      "`/permission list` includes `fwa:mail:send` (default FWA leader role + Administrator).",
